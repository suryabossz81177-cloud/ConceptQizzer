...
function initScienceLabSelector(){
  ...
  grid.querySelectorAll("[data-science-class]").forEach(btn=>btn.addEventListener("click",()=>{
    ...
    chapterGrid.querySelectorAll("[data-vsl-index]").forEach(chBtn=>chBtn.addEventListener("click",()=>{
      ...
      expGrid.innerHTML=labs.map(l=>`<a class="scienceExperimentLink" href="${l.link||'#'}" data-lab-class="${cId}" data-lab-index="${ci}" data-lab-name="${escapeHTML(name)}" data-lab-link="${l.link||''}"><article class="scienceExperimentCard"><div class="experimentIcon">${l.icon}</div><div class="experimentMeta">EXPERIMENT ${l.n} • ${escapeHTML(l.tag)}</div><h3>${escapeHTML(l.title)}</h3><p>${escapeHTML(l.text)}</p><div class="experimentTap"><span>Open & Play Simulation</span><span>→</span></div></article></a>`).join("");
