...
/* Premium dashboard shortcut */
document.addEventListener("DOMContentLoaded",()=>{
  const cards=document.querySelectorAll(".dashboardCard");
  if(cards[3]){
    cards[3].addEventListener("click",()=>location.href="achievements.html");
  }
  if(cards[2]){
    cards[2].addEventListener("click",()=>{
      alert("Practice mode is coming next. For now, use Notes + Quiz to strengthen the chapter.");
    });
  }
});
