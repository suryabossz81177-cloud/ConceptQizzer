const quizDatabase = {
  class6: {
    science: {"6-science-the-wonderful-world-of-science": {"easy":[{"question":"Science begins most directly with…","options":["curiosity and questions","memorising every fact","guessing without testing","copying a result"],"answer":0},{"question":"An observation is…","options":["a conclusion","something noticed or measured","a random guess","a final answer"],"answer":1},{"question":"A prediction tells us…","options":["what may happen","what already happened for sure","which answer to copy","that evidence is unnecessary"],"answer":0},{"question":"A fair test usually changes…","options":["everything at once","one main factor","nothing","the result"],"answer":1},{"question":"Evidence can include…","options":["measurements","only opinions","only guesses","nothing written down"],"answer":0},{"question":"Why repeat a test?","options":["to hide errors","to compare whether a result is consistent","to change every variable","to avoid recording"],"answer":1},{"question":"A hypothesis should be…","options":["untestable","a testable possible explanation","a copied result","a secret"],"answer":1},{"question":"If a prediction is wrong, a scientist should…","options":["hide it","learn from the result","change the data","stop asking questions"],"answer":1},{"question":"Which is a variable?","options":["something that can change","a fixed conclusion","a textbook page","a score only"],"answer":0},{"question":"The final explanation should be based on…","options":["evidence","hope","guesswork only","the expected answer"],"answer":0},{"question":"Which is a measurable observation?","options":["The plant is 18 cm tall","The plant looks nice","The plant is interesting","The plant should grow"],"answer":0},{"question":"Which sequence matches scientific thinking?","options":["Observe → Ask → Predict → Test → Record → Explain","Guess → Copy → Stop","Answer → Hide → Test","Memorise → Ignore → Finish"],"answer":0},{"question":"A pen has very little ink. Which factor is being changed?","options":["Ink level","Question wording","Conclusion","Observation"],"answer":0},{"question":"A plant grows 2 cm in a week. What is 2 cm?","options":["A measurement","A hypothesis","A question","An opinion"],"answer":0},{"question":"Why should results be written down?","options":["To make comparisons possible","To make guesses look correct","To avoid repeating tests","To remove evidence"],"answer":0},{"question":"A scientist notices a torch fails sometimes. What should come next?","options":["Ask a testable question","Immediately replace all parts","Ignore the observation","Choose a random answer"],"answer":0},{"question":"Which statement is a prediction?","options":["The plant may grow faster in suitable light","The plant is 15 cm tall","The leaf is green","The pot is 12 cm wide"],"answer":0},{"question":"Which is an explanation rather than an observation?","options":["The pen tip may be blocked","The pen tip looks dry","The pen is 15 cm long","The ink is blue"],"answer":0},{"question":"What should a fair test avoid?","options":["Changing several important factors together","Recording measurements","Repeating trials","Making a prediction"],"answer":0},{"question":"A result is different from what you expected. It should be…","options":["recorded honestly","deleted","changed","hidden"],"answer":0},{"question":"Which item can be measured in centimetres?","options":["Plant height","Ink colour","A question","A hypothesis"],"answer":0},{"question":"Which action helps compare two trials fairly?","options":["Keep important conditions steady","Change everything","Ignore the starting conditions","Use different measurements each time"],"answer":0},{"question":"What does “repeat” mean in an investigation?","options":["Run the test again","Change the conclusion","Erase the result","Stop after one observation"],"answer":0},{"question":"What can a surprising result do?","options":["Lead to a new question","Prove every idea false","Make evidence useless","End science"],"answer":0},{"question":"A scientific conclusion should be…","options":["supported by the evidence","based only on the prediction","based on opinion","unrelated to the test"],"answer":0}],"medium":[{"question":"Why is changing many factors at once a problem?","options":["It makes the test faster","You cannot tell which factor caused the result","It always gives a correct result","It removes observations"],"answer":1},{"question":"In a plant-light investigation, what should be measured?","options":["the result such as growth","the scientist’s opinion","the question itself","nothing"],"answer":0},{"question":"Which is the best scientific question?","options":["Why is everything strange?","Does changing light affect plant growth?","Is science good?","Can I guess the answer?"],"answer":1},{"question":"A conclusion conflicts with a prediction. What should happen?","options":["The evidence should be examined honestly","The data should be changed","The test should be hidden","Science has failed"],"answer":0},{"question":"What should be kept steady in a fair test?","options":["important control conditions","the result","the hypothesis only","the evidence"],"answer":0},{"question":"Which sequence is best?","options":["Guess → copy → stop","Observe → Ask → Predict → Test → Record → Explain","Test → hide → guess","Answer → change evidence"],"answer":1},{"question":"Why record results?","options":["So results can be compared later","So evidence can be forgotten","To make guesses look correct","To avoid measurements"],"answer":0},{"question":"What makes a question testable?","options":["It can be investigated with observations or measurements","It has no possible evidence","It depends only on opinion","It cannot be repeated"],"answer":0},{"question":"If a pen stops writing, a scientific approach is to…","options":["test possible causes one at a time","throw it away immediately","guess one cause and stop","change every condition"],"answer":0},{"question":"Why can an unexpected result be valuable?","options":["It can lead to a better question","It must be hidden","It is always an error","It cannot be evidence"],"answer":0},{"question":"A plant has enough water but poor light. Which factor is most directly worth changing?","options":["Light","The conclusion","The question","The record"],"answer":0},{"question":"A pen with a clear tip writes weakly at 10% ink. What is a sensible next test?","options":["Increase ink level while keeping the tip condition the same","Change ink and tip together","Change the question","Delete the result"],"answer":0},{"question":"A helicopter is tested at two blade lengths. What should remain constant?","options":["Paper mass and drop height","Blade length","The final conclusion","The question"],"answer":0},{"question":"Why measure fall time with a timer?","options":["It creates comparable numerical evidence","It proves the hypothesis automatically","It removes all variation","It changes blade length"],"answer":0},{"question":"A ramp test uses smooth and rough surfaces. What is the outcome variable?","options":["Travel distance","Surface type","The prediction","The chapter title"],"answer":0},{"question":"A student gets 40 cm, 42 cm and 41 cm. What does this suggest?","options":["The results are fairly consistent","The test cannot be repeated","The data must be wrong","No measurement was made"],"answer":0},{"question":"Why should the same starting point be used in a ramp comparison?","options":["To make the distance comparison fair","To make the cart heavier","To change the surface","To hide variation"],"answer":0},{"question":"Which is the strongest evidence for a claim?","options":["Repeated measurements that show a similar pattern","One unsupported opinion","A guess","A copied sentence"],"answer":0},{"question":"If two variables change and the result improves, what is difficult to know?","options":["Which variable caused the improvement","Whether a question existed","Whether a record was made","What the variables were"],"answer":0},{"question":"A scientist changes light from 4 h to 8 h but also changes water. What is the main issue?","options":["Two important factors changed","The plant was measured","A prediction was made","The result was recorded"],"answer":0},{"question":"Which statement is most scientifically careful?","options":["Under these conditions, the plant grew more with suitable light","Light always improves every plant","The result proves all plants behave the same","No further evidence is needed"],"answer":0},{"question":"What is the purpose of a prediction?","options":["To state an expected outcome before testing","To replace evidence","To guarantee the result","To avoid measurement"],"answer":0},{"question":"What should happen after an unsupported hypothesis?","options":["Revise the explanation and test again","Change the evidence","Ignore the result","Stop asking questions"],"answer":0},{"question":"Which action improves repeatability?","options":["Using the same procedure and recording measurements","Changing the procedure each time","Ignoring controls","Guessing the result"],"answer":0},{"question":"Why is evidence more useful than a guess?","options":["It can be checked against observations or measurements","It is always correct","It needs no testing","It removes every uncertainty"],"answer":0}],"hard":[{"question":"A student changes light, water and soil at the same time. What is the main weakness?","options":["The cause of a growth change is difficult to identify","Plants cannot be studied","Measurements are impossible","The question is too short"],"answer":0},{"question":"Which prediction is most testable?","options":["More suitable light may increase growth over seven days","Plants are interesting","Science is useful","Plants are beautiful"],"answer":0},{"question":"Why is “record what happened” important?","options":["Evidence should not be altered to match a prediction","Predictions are always correct","Scientists never make mistakes","Results do not matter"],"answer":0},{"question":"A paper helicopter falls in 1.8 s, then 1.9 s, then 1.8 s. What is useful?","options":["The result is reasonably consistent","No conclusion can ever be made","Measurements are useless","The helicopter did not move"],"answer":0},{"question":"If changing one variable produces no change, the next step could be…","options":["Improve the question or test another condition","Change the result","Hide the observation","Declare science impossible"],"answer":0},{"question":"Which best distinguishes observation from explanation?","options":["“The leaf is yellow” is an observation; “it lacks water” is an explanation","They are always identical","Explanations are measurements","Observations are guesses"],"answer":0},{"question":"A fair comparison of two surfaces should keep what constant?","options":["Important conditions such as the object and starting setup","Every result","The conclusion","The surface itself"],"answer":0},{"question":"What is the role of evidence?","options":["It helps evaluate an explanation","It guarantees every hypothesis","It replaces questions","It removes uncertainty completely"],"answer":0},{"question":"If two trials disagree strongly, a scientist should…","options":["Check the procedure and repeat carefully","Choose the nicer result","Delete one trial","Guess the average without checking"],"answer":0},{"question":"Which is the strongest scientific habit?","options":["Being willing to change an idea when evidence demands it","Never changing an idea","Only accepting expected results","Avoiding measurements"],"answer":0},{"question":"A plant grows 5 cm under one light condition and 2 cm under another, with other factors controlled. What does this support?","options":["Light condition may affect growth in this setup","Light always controls every plant","No evidence was collected","The result is meaningless"],"answer":0},{"question":"A pen writes strongly at 80% ink but weakly at 20%, with the same clear tip. What is the strongest conclusion?","options":["Ink level affected writing strength in this test","Ink level always determines writing everywhere","The pen is broken","No comparison was possible"],"answer":0},{"question":"A ruler shows a plant grew from 20 cm to 24 cm. What is the growth?","options":["4 cm","20 cm","24 cm","44 cm"],"answer":0},{"question":"A ramp cart travels 48 cm on smooth and 25 cm on carpet from the same start. What is supported?","options":["Surface condition affected travel distance in this setup","Carpet always stops every cart","Slope has no effect","Distance was not measured"],"answer":0},{"question":"A student records only the best helicopter fall time. What is the problem?","options":["Selective reporting can mislead the comparison","The timer is unnecessary","The helicopter cannot be tested","A prediction was made"],"answer":0},{"question":"Which design best tests watering level?","options":["Change watering while keeping light, nutrients and plant type steady","Change watering and light together","Use a different plant each time","Measure only one plant once"],"answer":0},{"question":"Why is a control condition useful?","options":["It provides a comparison point","It guarantees the result","It replaces measurement","It changes the hypothesis"],"answer":0},{"question":"If a model gives a high plant-health score, what should a learner do?","options":["Check which inputs produced it and compare with other runs","Assume reality is identical","Stop testing","Change the output manually"],"answer":0},{"question":"What does a repeated unexpected result suggest?","options":["It may represent a real pattern worth investigating","It must be deleted","The evidence is useless","The prediction becomes true"],"answer":0},{"question":"A question asks how slope affects distance. Which is the outcome?","options":["Distance travelled","Slope angle","Cart colour","Prediction"],"answer":0},{"question":"A student changes blade length from 8 cm to 12 cm but also doubles mass. What is the weakness?","options":["The effects of blade length and mass cannot be separated","The result cannot be measured","The helicopter has no variables","The question is impossible"],"answer":0},{"question":"Why should units be written with measurements?","options":["They tell what the numerical value represents","They make the value larger","They replace evidence","They remove error"],"answer":0},{"question":"Which conclusion is too broad?","options":["This setup proves smooth surfaces always make every object travel farther","The smooth surface produced greater distance in these trials","The result may depend on conditions","More tests could be useful"],"answer":0},{"question":"What should be done if measurement technique may be causing variation?","options":["Standardise the measurement method and repeat","Delete the unusual data","Change the conclusion first","Stop recording"],"answer":0},{"question":"A scientific explanation is strongest when it…","options":["Connects the result with evidence and a plausible cause","Repeats the prediction","Uses only opinion","Ignores unexpected data"],"answer":0}],"toohard":[{"question":"A student claims light caused growth because the sunny plant grew more, but also used different soil. What is missing?","options":["Control of another important variable","A title","A question","An observation"],"answer":0},{"question":"Which design best tests blade length?","options":["Change blade length while keeping paper mass and drop height steady","Change blade length, mass and height together","Use different objects each time","Do one trial only"],"answer":0},{"question":"Why can a prediction be useful even if it is wrong?","options":["It gives a clear expectation that can be compared with evidence","It proves the opposite is true","It removes the need for testing","It is always correct later"],"answer":0},{"question":"A result changes after only the surface is changed. What does this support?","options":["Surface may affect the measured outcome in that setup","Surface can never matter","Slope caused every result","Evidence is unnecessary"],"answer":0},{"question":"Which conclusion is most careful?","options":["Under these conditions, the smoother surface produced a greater simulated distance","Smooth surfaces always work best everywhere","The result proves every surface behaves the same","No evidence is needed"],"answer":0},{"question":"What is the best reason to change one factor at a time?","options":["It improves the ability to link a change with an outcome","It makes the experiment look longer","It guarantees the hypothesis","It removes all uncertainty"],"answer":0},{"question":"If a hypothesis is not supported, which action is scientific?","options":["Revise the explanation and test again","Change the measurements","Ignore the result","Declare the experiment useless"],"answer":0},{"question":"Which is an example of measurable evidence?","options":["Fall time of 1.82 seconds","The helicopter looked cool","I think it fell quickly","It seemed interesting"],"answer":0},{"question":"Why should unexpected results be recorded?","options":["They may reveal a new variable or question","They make the experiment invalid","They must be embarrassing","They never contain information"],"answer":0},{"question":"A good scientific question should connect…","options":["a factor that can be changed with an outcome that can be observed","two opinions only","a guess with no measurement","a conclusion with no test"],"answer":0},{"question":"A plant study uses three plants with the same soil but different light durations. What is the independent variable?","options":["Light duration","Plant height","Soil type","Final conclusion"],"answer":0},{"question":"A plant study records height each day. What is the dependent variable?","options":["Plant height","Light duration","Watering setting","Pot material"],"answer":0},{"question":"Why use more than one plant in a comparison?","options":["It can reduce the influence of an unusual individual result","It guarantees the hypothesis","It removes the need for controls","It changes the light"],"answer":0},{"question":"A pen has a clear tip but 5% ink. Which test best checks whether low ink is the cause?","options":["Refill ink while keeping the same tip and writing conditions","Change ink and tip together","Use a different pen","Change the question"],"answer":0},{"question":"A helicopter has longer blades but also thicker paper. Why is interpretation difficult?","options":["Two factors changed together","No measurement exists","The result is automatically false","The drop height is irrelevant"],"answer":0},{"question":"A ramp test uses 10°, 20° and 30°. What should be recorded besides angle?","options":["Travel distance for each trial","Only the prediction","Only the final conclusion","The colour of the cart"],"answer":0},{"question":"If roughness reduces travel distance in repeated trials, the safest claim is…","options":["Roughness reduced distance under the tested conditions","Rough surfaces always stop everything","Slope never matters","The result proves all materials behave identically"],"answer":0},{"question":"Why should the same ruler or measuring method be used across plant trials?","options":["To keep measurement method consistent","To change plant height","To make the prediction true","To avoid units"],"answer":0},{"question":"A surprising result occurs only once. What is a sensible response?","options":["Repeat the test and check the procedure","Delete it immediately","Treat it as universal","Change the data"],"answer":0},{"question":"A simulation shows a result after changing one input. What should be checked next?","options":["Whether the result changes consistently when the input is varied","Whether the screen looks attractive","Whether the prediction is automatically true","Whether measurements can be ignored"],"answer":0},{"question":"Which statement about models is most careful?","options":["A model represents a situation and has limits","A model is always identical to reality","A model never needs testing","A model replaces evidence"],"answer":0},{"question":"A student changes the setup after seeing an unexpected result and reports only the new run. What is the issue?","options":["The evidence was selectively reported","The question was measurable","The result was repeated","The units were correct"],"answer":0},{"question":"A fair test of soil nutrients should control…","options":["Light, watering and plant type","The nutrient level itself","The final result","The prediction"],"answer":0},{"question":"Which evidence best supports a causal claim?","options":["A controlled comparison with repeated measurements","A single opinion","A guess","A decorative diagram"],"answer":0},{"question":"After five similar trials, one result is very different. What should be done?","options":["Check the procedure and consider repeating the unusual trial","Delete it without checking","Use only the unusual result","Change the units"],"answer":0}],"extreme":[{"question":"A model predicts a plant health score. What should a learner do to test the model fairly?","options":["Change one input at a time and compare repeated runs","Change every input randomly","Accept the first result","Ignore the output"],"answer":0},{"question":"A simulated result supports a prediction. What can we safely say?","options":["The prediction matched the result under the tested conditions","The prediction is universally true","No more testing is needed","The model proves reality exactly"],"answer":0},{"question":"Why is “under these conditions” important in a conclusion?","options":["It limits the claim to the tested situation","It makes the conclusion useless","It removes evidence","It prevents repetition"],"answer":0},{"question":"If changing two variables gives a better result, what is the next fair test?","options":["Separate the variables and test each effect","Keep both changes and claim both caused it","Remove all measurements","Choose the expected answer"],"answer":0},{"question":"A result is unusual once but normal in four repeats. What should receive more weight?","options":["The repeated pattern, while still noting the unusual trial","Only the unusual trial","Neither result","A guess"],"answer":0},{"question":"Which statement best describes scientific knowledge?","options":["It can improve as new evidence appears","It never changes","It depends only on authority","It avoids uncertainty"],"answer":0},{"question":"A learner changes the experimental setup after seeing the result and then reports only the successful run. What is the problem?","options":["The evidence was selectively reported","The question was too short","The result was measured","The experiment had a title"],"answer":0},{"question":"What makes a simulation educationally useful?","options":["It lets learners change inputs, observe outputs and connect them to a concept","It gives answers without interaction","It hides the variables","It prevents retesting"],"answer":0},{"question":"Which is the strongest next step after a surprising result?","options":["Ask what additional test could distinguish possible explanations","Immediately pick one explanation","Delete the result","Stop collecting evidence"],"answer":0},{"question":"Which principle links all five laboratory activities?","options":["Make a prediction, change a condition, observe the result and explain the evidence","Memorise the expected answer","Avoid changing variables","Never repeat a test"],"answer":0},{"question":"A plant model gives health scores of 82, 84 and 83 under the same inputs. What does this suggest?","options":["The model output is stable for those inputs","The model is universally true","No testing is needed","The inputs are irrelevant"],"answer":0},{"question":"A plant model gives 90 with high light and 40 with low light, while all other inputs stay fixed. What does the comparison support?","options":["Light level affects the model output","Light always affects every real plant identically","No relationship exists","The model has no variables"],"answer":0},{"question":"A real experiment disagrees with a simulation. What is the best scientific response?","options":["Compare assumptions, conditions and measurements before revising the model or explanation","Declare the real experiment wrong","Delete the simulation","Choose whichever result looks better"],"answer":0},{"question":"A student wants to claim that a 30 cm plant grew because of light. What additional evidence strengthens the claim?","options":["A controlled comparison with similar plants and controlled watering/nutrients","A single photograph","A prediction alone","A different soil type"],"answer":0},{"question":"A pen writes weakly at 20% ink and strongly at 80%, but the 80% test also used a new nib. What is the strongest criticism?","options":["Ink level and nib condition were confounded","The ink cannot be measured","The pen has no variables","The result is automatically true"],"answer":0},{"question":"A helicopter with 12 cm blades falls more slowly than one with 8 cm blades, but the heavier one had 12 cm blades. What should be tested next?","options":["Blade length while keeping mass constant","Mass and blade length together again","Only the prediction","No further test"],"answer":0},{"question":"A ramp experiment measures 44, 45, 44 cm on smooth and 30, 29, 31 cm on carpet. What is the strongest conclusion?","options":["The smooth setup produced greater distance in these trials","Smooth surfaces always produce greater distance","Carpet has no effect","The measurements are inconsistent"],"answer":0},{"question":"Why is a baseline/control condition valuable in a multi-condition experiment?","options":["It gives a reference against which changes can be compared","It guarantees the desired result","It eliminates every variable","It replaces repeated trials"],"answer":0},{"question":"If two explanations fit the same observation, what should a scientist seek?","options":["A test whose predicted outcomes differ between the explanations","A longer definition","A random choice","A decorative figure"],"answer":0},{"question":"Which statement best reflects scientific uncertainty?","options":["Evidence can support a conclusion without making it universally certain","Uncertainty means evidence is useless","A correct prediction removes all uncertainty","Science never uses measurements"],"answer":0},{"question":"A result changes only when two factors are changed together. What is the next logical step?","options":["Test the factors separately and in controlled combinations","Assume both are equally responsible","Ignore the interaction","Change the measurement units"],"answer":0},{"question":"Why should units and measurement method be reported?","options":["They allow another person to understand and compare the evidence","They make the result automatically correct","They remove the need for controls","They prove the hypothesis"],"answer":0},{"question":"A learner repeats an experiment until getting the expected result and ignores other trials. What is wrong?","options":["The evidence is selectively reported","The experiment is too short","The prediction is always wrong","Repeating is never useful"],"answer":0},{"question":"A scientific model predicts one outcome but repeated observations show another. What is the most useful next step?","options":["Investigate which assumption or variable may explain the mismatch","Change the observations","Hide the mismatch","Declare the model perfect"],"answer":0},{"question":"Which conclusion is strongest after a controlled investigation?","options":["A conclusion that matches the measured pattern and states the conditions","A universal claim from one trial","An opinion without data","A conclusion chosen before testing"],"answer":0}]}},
    mathematics: {
      "patterns-in-mathematics": {
        easy: [
{
question: "What is a pattern?",
options: [
"A random arrangement",
"A repeated rule or design",
"Only a drawing",
"A type of number"
],
answer: 1
},
{
question: "Which of these is a number pattern?",
options: [
"2, 4, 6, 8",
"A, B, C",
"Red, Blue",
"Circle, Square"
],
answer: 0
},
{
question: "What comes next? 5, 10, 15, ?",
options: [
"18",
"20",
"25",
"30"
],
answer: 1
},
{
question: "What comes next? 3, 6, 9, ?",
options: [
"10",
"11",
"12",
"15"
],
answer: 2
},
{
question: "Which pattern increases by 2?",
options: [
"2, 4, 6, 8",
"5, 10, 15",
"3, 6, 9",
"1, 4, 9"
],
answer: 0
},
{
question: "What comes next? A, C, E, ?",
options: [
"F",
"G",
"H",
"I"
],
answer: 1
},
{
question: "Which is an even number pattern?",
options: [
"1,3,5,7",
"2,4,6,8",
"5,10,15",
"3,6,9"
],
answer: 1
},
{
question: "What comes next? 7, 14, 21, ?",
options: [
"24",
"27",
"28",
"30"
],
answer: 2
},
{
question: "Which shape pattern repeats?",
options: [
"Circle, Square, Circle, Square",
"Circle, Triangle, Star",
"Square, Rectangle",
"None"
],
answer: 0
},
{
question: "Patterns help us to...",
options: [
"Predict the next term",
"Erase numbers",
"Draw maps",
"Write essays"
],
answer: 0
}
],
        medium: [
  {
    question: "What comes next? 12, 18, 24, 30, ?",
    options: ["34", "36", "38", "40"],
    answer: 1
  },
  {
    question: "Which rule generates 6, 11, 16, 21, 26?",
    options: ["Add 4", "Add 5", "Add 6", "Multiply by 2"],
    answer: 1
  },
  {
    question: "What is the missing number? 4, 8, 16, 32, ?",
    options: ["48", "56", "64", "72"],
    answer: 2
  },
  {
    question: "What comes next? 30, 27, 24, 21, ?",
    options: ["19", "18", "17", "16"],
    answer: 1
  },
  {
    question: "Which sequence follows a multiplication rule?",
    options: [
      "5, 10, 15, 20",
      "3, 6, 12, 24",
      "10, 15, 20, 25",
      "8, 12, 16, 20"
    ],
    answer: 1
  },
  {
    question: "What is the next term? 1, 4, 7, 10, ?",
    options: ["12", "13", "14", "15"],
    answer: 1
  },
  {
    question: "Which number completes the pattern? 100, 90, 80, 70, ?",
    options: ["65", "60", "55", "50"],
    answer: 1
  },
  {
    question: "What is the next term? 2, 6, 18, 54, ?",
    options: ["108", "144", "162", "216"],
    answer: 2
  },
  {
    question: "A pattern starts at 9 and increases by 7 each time. What is the fourth term?",
    options: ["23", "28", "30", "37"],
    answer: 1
  },
  {
    question: "What comes next? 1, 3, 5, 7, 9, ?",
    options: ["10", "11", "12", "13"],
    answer: 1
  }
],

hard: [
  {
    question: "What comes next? 2, 6, 12, 20, 30, ?",
    options: ["36", "40", "42", "44"],
    answer: 2
  },
  {
    question: "The differences in a sequence are 3, 5, 7, 9. If the first term is 2, what is the fifth term?",
    options: ["22", "24", "26", "28"],
    answer: 1
  },
  {
    question: "What is the missing term? 3, 9, 27, 81, ?",
    options: ["162", "216", "243", "324"],
    answer: 2
  },
  {
    question: "Which rule generates 4, 9, 19, 39, 79?",
    options: ["×2 + 1", "×2 - 1", "×3 + 1", "+5"],
    answer: 0
  },
  {
    question: "What comes next? 50, 45, 37, 26, 12, ?",
    options: ["0", "-3", "-5", "-7"],
    answer: 2
  },
  {
    question: "A sequence is 1, 4, 9, 16, 25. What is the next term?",
    options: ["30", "32", "36", "40"],
    answer: 2
  },
  {
    question: "What is the missing number? 5, 11, 23, 47, ?",
    options: ["91", "93", "95", "97"],
    answer: 2
  },
  {
    question: "If a sequence follows ×2 + 3, starting with 2, what is the fourth term?",
    options: ["25", "29", "35", "41"],
    answer: 1
  },
  {
    question: "What comes next? 64, 32, 16, 8, ?",
    options: ["6", "4", "2", "1"],
    answer: 1
  },
  {
    question: "Which sequence has differences increasing by 2?",
    options: [
      "2, 5, 10, 17, 26",
      "2, 4, 6, 8, 10",
      "3, 6, 9, 12, 15",
      "1, 2, 4, 8, 16"
    ],
    answer: 0
  },
  {
    question: "What is the 6th term of 4, 8, 12, 16, ...?",
    options: ["20", "24", "28", "32"],
    answer: 1
  },
  {
    question: "What is the next number? 1, 2, 4, 7, 11, ?",
    options: ["15", "16", "17", "18"],
    answer: 1
  },
  {
    question: "Which number completes the pattern? 81, 27, 9, 3, ?",
    options: ["0", "1", "2", "6"],
    answer: 1
  },
  {
    question: "What is the next term? 7, 14, 28, 56, ?",
    options: ["84", "98", "112", "120"],
    answer: 2
  },
  {
    question: "A pattern begins 3, 6, 11, 18, 27. What is the next term?",
    options: ["36", "38", "40", "42"],
    answer: 1
  }
],

tooHard: [
  {
    question: "What comes next? 2, 5, 11, 23, 47, ?",
    options: ["93", "94", "95", "96"],
    answer: 2
  },
  {
    question: "What is the missing term? 1, 4, 10, 22, 46, ?",
    options: ["90", "92", "94", "96"],
    answer: 2
  },
  {
    question: "A sequence follows ×3 - 1. Starting with 2, what is the fifth term?",
    options: ["122", "160", "161", "162"],
    answer: 2
  },
  {
    question: "What comes next? 3, 8, 18, 38, 78, ?",
    options: ["154", "156", "158", "160"],
    answer: 2
  },
  {
    question: "Find the missing number: 2, 6, 12, 20, 30, 42, ?",
    options: ["54", "56", "58", "60"],
    answer: 1
  },
  {
    question: "The differences are 2, 4, 6, 8, 10. If the first term is 1, what is the sixth term?",
    options: ["31", "36", "37", "41"],
    answer: 2
  },
  {
    question: "What is the next term? 1, 3, 6, 10, 15, 21, ?",
    options: ["27", "28", "29", "30"],
    answer: 1
  },
  {
    question: "Which rule generates 2, 7, 17, 37, 77?",
    options: ["×2 + 3", "×2 + 1", "×3 + 1", "+5"],
    answer: 0
  },
  {
    question: "What is the missing number? 4, 10, 22, 46, 94, ?",
    options: ["186", "188", "190", "192"],
    answer: 2
  },
  {
    question: "What comes next? 100, 50, 25, 12.5, ?",
    options: ["5", "6.25", "7.5", "8"],
    answer: 1
  },
  {
    question: "If the nth term is 2n + 3, what is the 12th term?",
    options: ["25", "27", "29", "30"],
    answer: 1
  },
  {
    question: "What is the next term? 5, 10, 20, 40, 80, ?",
    options: ["120", "140", "160", "180"],
    answer: 2
  },
  {
    question: "A sequence begins 2, 4, 8, 16. What is the 10th term?",
    options: ["256", "512", "1024", "2048"],
    answer: 2
  },
  {
    question: "What is the missing number? 1, 8, 27, 64, ?, 216",
    options: ["100", "125", "144", "150"],
    answer: 1
  },
  {
    question: "What comes next? 6, 13, 27, 55, 111, ?",
    options: ["221", "222", "223", "224"],
    answer: 2
  },
  {
    question: "The differences of a pattern are 4, 7, 10, 13. If the first term is 2, what is the fifth term?",
    options: ["30", "32", "36", "40"],
    answer: 1
  },
  {
    question: "What is the next term? 2, 3, 5, 8, 13, 21, ?",
    options: ["32", "34", "35", "36"],
    answer: 1
  },
  {
    question: "A sequence follows ×2 - 1. Starting at 5, what is the sixth term?",
    options: ["121", "129", "145", "161"],
    answer: 1
  },
  {
    question: "What is the missing number? 10, 18, 34, 66, ?",
    options: ["128", "130", "132", "134"],
    answer: 1
  },
  {
    question: "What comes next? 2, 9, 28, 65, 126, ?",
    options: ["215", "217", "219", "221"],
    answer: 1
  }
],

extreme: [
  {
    question: "What is the next term? 2, 6, 12, 20, 30, 42, ?",
    options: ["54", "56", "58", "60"],
    answer: 1
  },
  {
    question: "What comes next? 1, 2, 6, 24, 120, ?",
    options: ["480", "600", "720", "840"],
    answer: 2
  },
  {
    question: "Find the missing term: 2, 5, 14, 41, 122, ?",
    options: ["243", "365", "366", "367"],
    answer: 1
  },
  {
    question: "What is the next term? 3, 8, 18, 38, 78, ?",
    options: ["156", "158", "160", "162"],
    answer: 1
  },
  {
    question: "A sequence follows ×2 + n, where n increases by 1 each time. Starting with 1, what are the first four terms?",
    options: [
      "1, 3, 8, 19",
      "1, 3, 7, 16",
      "1, 4, 10, 23",
      "1, 2, 5, 12"
    ],
    answer: 0
  },
  {
    question: "What is the next term? 1, 3, 7, 15, 31, 63, ?",
    options: ["95", "111", "127", "129"],
    answer: 2
  },
  {
    question: "What comes next? 2, 10, 30, 68, 130, ?",
    options: ["200", "210", "222", "240"],
    answer: 2
  },
  {
    question: "What is the missing term? 5, 11, 23, 47, 95, ?",
    options: ["189", "190", "191", "192"],
    answer: 2
  },
  {
    question: "If the nth term is n² + n, what is the 10th term?",
    options: ["100", "110", "120", "121"],
    answer: 1
  },
  {
    question: "What is the next term? 1, 4, 10, 22, 46, 94, ?",
    options: ["186", "188", "190", "192"],
    answer: 2
  },
  {
    question: "A pattern has first differences 2, 5, 10, 17, 26. What is the next difference?",
    options: ["35", "36", "37", "38"],
    answer: 2
  },
  {
    question: "What is the 8th term of 2, 4, 8, 16, 32, ...?",
    options: ["128", "192", "256", "512"],
    answer: 2
  },
  {
    question: "What is the missing number? 1, 5, 13, 29, 61, ?",
    options: ["121", "122", "123", "125"],
    answer: 2
  },
  {
    question: "What comes next? 4, 9, 19, 39, 79, ?",
    options: ["157", "158", "159", "160"],
    answer: 2
  },
  {
    question: "What is the next term? 2, 9, 28, 65, 126, ?",
    options: ["215", "217", "219", "221"],
    answer: 1
  },
  {
    question: "If the nth term is n³ + 1, what is the 6th term?",
    options: ["215", "216", "217", "218"],
    answer: 2
  },
  {
    question: "A sequence is 3, 6, 12, 24, 48. What is the 9th term?",
    options: ["384", "512", "768", "1024"],
    answer: 2
  },
  {
    question: "What is the missing number? 7, 15, 31, 63, 127, ?",
    options: ["253", "254", "255", "256"],
    answer: 2
  },
  {
    question: "The differences are 1, 4, 9, 16, 25. If the first term is 3, what is the sixth term?",
    options: ["52", "58", "61", "63"],
    answer: 2
  },
  {
    question: "What comes next? 2, 7, 17, 37, 77, ?",
    options: ["151", "153", "155", "157"],
    answer: 1
  },
  {
    question: "What is the next term? 1, 8, 27, 64, 125, ?",
    options: ["196", "216", "225", "243"],
    answer: 1
  },
  {
    question: "A sequence follows ×3 - 2. Starting with 2, what is the sixth term?",
    options: ["242", "484", "486", "728"],
    answer: 2
  },
  {
    question: "What comes next? 10, 20, 40, 80, 160, ?",
    options: ["240", "280", "320", "360"],
    answer: 2
  },
  {
    question: "What is the missing term? 2, 12, 36, 80, 150, ?",
    options: ["240", "252", "258", "270"],
    answer: 1
  },
  {
    question: "Which rule best describes 2, 6, 18, 54, 162?",
    options: [
      "Add consecutive multiples of 4",
      "Multiply by 3",
      "Multiply by 2 and add 2",
      "Square each term"
    ],
    answer: 1
  }
]
      },

"lines-and-angles": {

  easy: [
    {
      question: "Which figure has two endpoints?",
      options: [
        "Line",
        "Line segment",
        "Ray",
        "Point"
      ],
      answer: 1
    },

    {
      question: "Which figure has exactly one endpoint?",
      options: [
        "Line",
        "Line segment",
        "Ray",
        "Point"
      ],
      answer: 2
    },

    {
      question: "What are three or more points on the same straight line called?",
      options: [
        "Parallel points",
        "Collinear points",
        "Angular points",
        "Reflex points"
      ],
      answer: 1
    },

    {
      question: "The common endpoint of the two rays forming an angle is called the:",
      options: [
        "Arm",
        "Side",
        "Vertex",
        "Line"
      ],
      answer: 2
    },

    {
      question: "Which angle measures exactly 90°?",
      options: [
        "Acute angle",
        "Right angle",
        "Obtuse angle",
        "Straight angle"
      ],
      answer: 1
    },

    {
      question: "Which of these is an acute angle?",
      options: [
        "45°",
        "90°",
        "120°",
        "180°"
      ],
      answer: 0
    },

    {
      question: "Which of these is an obtuse angle?",
      options: [
        "30°",
        "75°",
        "90°",
        "120°"
      ],
      answer: 3
    },

    {
      question: "A straight angle measures:",
      options: [
        "90°",
        "120°",
        "180°",
        "360°"
      ],
      answer: 2
    },

    {
      question: "Which angle is greater than 180° but less than 360°?",
      options: [
        "Acute",
        "Right",
        "Obtuse",
        "Reflex"
      ],
      answer: 3
    },

    {
      question: "In ∠ABC, which letter represents the vertex?",
      options: [
        "A",
        "B",
        "C",
        "None"
      ],
      answer: 1
    }
  ],

  medium: [
    {
      question: "Which statement about a ray is correct?",
      options: [
        "It has no endpoint",
        "It has two endpoints",
        "It has one endpoint",
        "It has three endpoints"
      ],
      answer: 2
    },

    {
      question: "Which angle is smaller than a right angle?",
      options: [
        "120°",
        "90°",
        "45°",
        "180°"
      ],
      answer: 2
    },

    {
      question: "Which angle is greater than 90° but less than 180°?",
      options: [
        "Acute",
        "Right",
        "Obtuse",
        "Reflex"
      ],
      answer: 2
    },

    {
      question: "How many endpoints does a line segment have?",
      options: [
        "0",
        "1",
        "2",
        "3"
      ],
      answer: 2
    },

    {
      question: "How many endpoints does a line have?",
      options: [
        "0",
        "1",
        "2",
        "4"
      ],
      answer: 0
    },

    {
      question: "If three points lie on the same straight line, they are:",
      options: [
        "Non-collinear",
        "Collinear",
        "Reflex",
        "Parallel"
      ],
      answer: 1
    },

    {
      question: "Which angle is larger?",
      options: [
        "25°",
        "40°",
        "75°",
        "15°"
      ],
      answer: 2
    },

    {
      question: "What is the measure of a straight angle?",
      options: [
        "45°",
        "90°",
        "180°",
        "270°"
      ],
      answer: 2
    },

    {
      question: "Which object can represent a line segment?",
      options: [
        "A ruler",
        "An endless road",
        "A single point",
        "A ray of light"
      ],
      answer: 0
    },

    {
      question: "In ∠PQR, the vertex is:",
      options: [
        "P",
        "Q",
        "R",
        "P and R"
      ],
      answer: 1
    }
  ],

  hard: [
    {
      question: "Which angle is exactly halfway between 90° and 180°?",
      options: [
        "125°",
        "135°",
        "145°",
        "150°"
      ],
      answer: 1
    },

    {
      question: "Which pair contains only acute angles?",
      options: [
        "30°, 60°",
        "90°, 45°",
        "100°, 80°",
        "120°, 30°"
      ],
      answer: 0
    },

    {
      question: "Which pair contains one acute and one obtuse angle?",
      options: [
        "40°, 120°",
        "90°, 180°",
        "30°, 60°",
        "180°, 270°"
      ],
      answer: 0
    },

    {
      question: "A 270° angle is a:",
      options: [
        "Right angle",
        "Straight angle",
        "Reflex angle",
        "Acute angle"
      ],
      answer: 2
    },

    {
      question: "Which statement is false?",
      options: [
        "A ray has one endpoint",
        "A line segment has two endpoints",
        "A line has two endpoints",
        "An angle has a vertex"
      ],
      answer: 2
    },

    {
      question: "Which angle has the widest opening?",
      options: [
        "30°",
        "90°",
        "120°",
        "60°"
      ],
      answer: 2
    },

    {
      question: "If an angle is greater than 180° but less than 360°, it is:",
      options: [
        "Acute",
        "Obtuse",
        "Straight",
        "Reflex"
      ],
      answer: 3
    },

    {
      question: "Which set contains three collinear points?",
      options: [
        "A, B and C on one straight line",
        "A, B and C forming a triangle",
        "Three points at different locations",
        "Three points forming an angle"
      ],
      answer: 0
    },

    {
      question: "Which angle is closest to a right angle?",
      options: [
        "10°",
        "40°",
        "85°",
        "150°"
      ],
      answer: 2
    },

    {
      question: "Which statement correctly compares 60° and 120°?",
      options: [
        "Both are acute",
        "60° is acute and 120° is obtuse",
        "Both are obtuse",
        "60° is obtuse and 120° is acute"
      ],
      answer: 1
    }
  ],

  tooHard: [
    {
      question: "Which classification is completely correct?",
      options: [
        "45° acute, 90° right, 135° obtuse",
        "90° acute, 180° obtuse, 270° straight",
        "120° acute, 180° reflex, 45° right",
        "30° obtuse, 90° reflex, 180° acute"
      ],
      answer: 0
    },

    {
      question: "Which sequence contains only angles greater than 90° and less than 180°?",
      options: [
        "100°, 120°, 150°",
        "45°, 100°, 150°",
        "90°, 120°, 180°",
        "120°, 180°, 150°"
      ],
      answer: 0
    },

    {
      question: "Which statement best distinguishes a line from a ray?",
      options: [
        "A line has two endpoints",
        "A ray has no endpoint",
        "A line extends in both directions while a ray extends in one",
        "A ray extends in both directions"
      ],
      answer: 2
    },

    {
      question: "Which angle is exactly twice a right angle?",
      options: [
        "45°",
        "90°",
        "180°",
        "270°"
      ],
      answer: 2
    },

    {
      question: "Which angle is exactly three times a right angle?",
      options: [
        "180°",
        "270°",
        "300°",
        "360°"
      ],
      answer: 1
    },

    {
      question: "Which option correctly identifies the vertex in ∠XYZ?",
      options: [
        "X",
        "Y",
        "Z",
        "X and Z"
      ],
      answer: 1
    },

    {
      question: "Which group contains one acute, one right and one obtuse angle?",
      options: [
        "30°, 90°, 120°",
        "90°, 180°, 270°",
        "45°, 60°, 80°",
        "100°, 120°, 150°"
      ],
      answer: 0
    },

    {
      question: "Which is impossible for a standard angle classification in this chapter?",
      options: [
        "30°",
        "90°",
        "180°",
        "400°"
      ],
      answer: 3
    },

    {
      question: "Which figure has a fixed measurable length?",
      options: [
        "Line",
        "Line segment",
        "Ray",
        "Point"
      ],
      answer: 1
    },

    {
      question: "Which statement about collinear points is correct?",
      options: [
        "They must form a triangle",
        "They lie on the same straight line",
        "They always form a right angle",
        "They must be endpoints of a ray"
      ],
      answer: 1
    }
  ],

  extreme: [
    {
      question: "Which option contains the correct classification of all five angles?",
      options: [
        "45° acute, 90° right, 120° obtuse, 180° straight, 270° reflex",
        "45° right, 90° acute, 120° straight, 180° reflex, 270° obtuse",
        "45° obtuse, 90° right, 120° acute, 180° reflex, 270° straight",
        "45° acute, 90° obtuse, 120° right, 180° straight, 270° reflex"
      ],
      answer: 0
    },

    {
      question: "Which statement correctly describes all three: line, segment and ray?",
      options: [
        "All have two endpoints",
        "Line has none, ray has one, segment has two",
        "Line has one, ray has two, segment has none",
        "All have one endpoint"
      ],
      answer: 1
    },

    {
      question: "If ∠ABC is 135°, which statement is correct?",
      options: [
        "It is acute and B is not the vertex",
        "It is right and A is the vertex",
        "It is obtuse and B is the vertex",
        "It is reflex and C is the vertex"
      ],
      answer: 2
    },

    {
      question: "Which angle is greater than a straight angle but smaller than a full turn?",
      options: [
        "45°",
        "90°",
        "180°",
        "270°"
      ],
      answer: 3
    },

    {
      question: "Which option contains only pairs of collinear points?",
      options: [
        "A-B on one line and C-D on another line",
        "A-B forming an angle and C-D forming a triangle",
        "A-B-C on one straight line",
        "Points forming a circle"
      ],
      answer: 2
    },

    {
      question: "Which statement about a line segment is always true?",
      options: [
        "It extends endlessly in both directions",
        "It has exactly two endpoints",
        "It has exactly one endpoint",
        "It cannot have a measurable length"
      ],
      answer: 1
    },

    {
      question: "Which angle is closest to 180° without being a straight angle?",
      options: [
        "179°",
        "90°",
        "45°",
        "270°"
      ],
      answer: 0
    },

    {
      question: "Which angle is closest to 360° without being a full turn?",
      options: [
        "180°",
        "270°",
        "359°",
        "90°"
      ],
      answer: 2
    },

    {
      question: "Which option correctly identifies the arms of ∠ABC?",
      options: [
        "AB and BC",
        "BA and BC",
        "A and C only",
        "Only B"
      ],
      answer: 1
    },

    {
      question: "Which complete statement is correct?",
      options: [
        "A line has two endpoints and a ray has none",
        "A line segment has no endpoints and an angle has no vertex",
        "A line extends both ways, a ray one way, and a segment has fixed endpoints",
        "All three are exactly the same"
      ],
      answer: 2
    }
  ]

      }
      
    }
  }
};

/*==================================================
  PACKAGE 13.1.3 — STEP 2
  CLASS 6 • MATHEMATICS • NUMBER PLAY
  Quiz database addition
==================================================*/

"use strict";

/*
  IMPORTANT:
  Paste this block at the VERY END of quizDatabase.js,
  AFTER the main `quizDatabase = { ... };` declaration.

  It does NOT replace the existing database.
*/

quizDatabase.class6 = quizDatabase.class6 || {};
quizDatabase.class6.mathematics =
  quizDatabase.class6.mathematics || {};

quizDatabase.class6.mathematics["number-play"] = {

  easy: [
    {
      question: "Which number is a palindrome?",
      options: ["1234", "1331", "1243", "1456"],
      answer: 1
    },
    {
      question: "What is the smallest 4-digit number that can be made using 2, 3, 4 and 7 once each?",
      options: ["2347", "2374", "2437", "2734"],
      answer: 0
    },
    {
      question: "What is the largest 4-digit number that can be made using 2, 3, 4 and 7 once each?",
      options: ["7234", "7423", "7432", "7342"],
      answer: 2
    },
    {
      question: "Which number is the same when read forwards and backwards?",
      options: ["2451", "3443", "4512", "5632"],
      answer: 1
    },
    {
      question: "In the Collatz process, what is done to an even number?",
      options: ["Multiply by 3", "Add 1", "Divide by 2", "Square it"],
      answer: 2
    },
    {
      question: "In the Collatz process, what is done to an odd number?",
      options: ["Divide by 2", "Multiply by 3 and add 1", "Subtract 1", "Multiply by 2"],
      answer: 1
    },
    {
      question: "What is estimation?",
      options: [
        "Finding an exact answer every time",
        "Finding a reasonable approximate value",
        "Guessing without thinking",
        "Ignoring the data"
      ],
      answer: 1
    },
    {
      question: "What is Kaprekar's constant for the suitable 4-digit routine?",
      options: ["6174", "6147", "6741", "6471"],
      answer: 0
    },
    {
      question: "Which is a useful reason for studying number puzzles?",
      options: [
        "They develop logical thinking",
        "They remove the need for calculation",
        "They make every answer exact",
        "They are only games"
      ],
      answer: 0
    },
    {
      question: "What should you do before deciding the rule of a number pattern?",
      options: [
        "Guess immediately",
        "Check several terms",
        "Choose the biggest number",
        "Ignore the first term"
      ],
      answer: 1
    }
  ],

  medium: [
    {
      question: "What is the difference between the largest and smallest numbers made from 2, 3, 4 and 7?",
      options: ["5085", "5185", "5095", "5075"],
      answer: 0
    },
    {
      question: "Which number is NOT palindromic?",
      options: ["1221", "2442", "3553", "1231"],
      answer: 3
    },
    {
      question: "Starting with 6, what is the next number in the Collatz process?",
      options: ["18", "12", "3", "2"],
      answer: 2
    },
    {
      question: "After 3 in the Collatz process, what comes next?",
      options: ["4", "8", "9", "10"],
      answer: 3
    },
    {
      question: "Which number is formed by arranging 8, 1, 5 and 3 in descending order?",
      options: ["1358", "8531", "8513", "8351"],
      answer: 1
    },
    {
      question: "Which number is formed by arranging 8, 1, 5 and 3 in ascending order?",
      options: ["1358", "1385", "1538", "1835"],
      answer: 0
    },
    {
      question: "A reasonable estimate should be:",
      options: [
        "Random",
        "Close enough for the purpose",
        "Always exact",
        "Always smaller than the real value"
      ],
      answer: 1
    },
    {
      question: "Which time shows repeated digits?",
      options: ["10:24", "11:11", "12:34", "09:27"],
      answer: 1
    },
    {
      question: "Why can a number string that looks like a time still be invalid?",
      options: [
        "Clock notation has fixed limits",
        "All repeated digits are invalid",
        "Times cannot contain zero",
        "Hours never use digits"
      ],
      answer: 0
    },
    {
      question: "Which is the best first step in a number-puzzle problem?",
      options: [
        "Look for the rule or condition",
        "Write a random answer",
        "Skip the data",
        "Use multiplication every time"
      ],
      answer: 0
    }
  ],

  hard: [
    {
      question: "What is 7432 − 2347?",
      options: ["5085", "5185", "4985", "5075"],
      answer: 0
    },
    {
      question: "What is 7432 + 2347?",
      options: ["9679", "9779", "9789", "9879"],
      answer: 1
    },
    {
      question: "Which sequence follows a multiply-by-2 rule?",
      options: ["3, 6, 12, 24", "2, 5, 8, 11", "20, 18, 16, 14", "1, 4, 7, 10"],
      answer: 0
    },
    {
      question: "Which sequence follows a subtract-5 rule?",
      options: ["30, 25, 20, 15", "2, 7, 12, 17", "3, 6, 12, 24", "4, 8, 16, 32"],
      answer: 0
    },
    {
      question: "If a number is 64 and the rule is divide by 2 repeatedly, what is the next term?",
      options: ["16", "32", "128", "62"],
      answer: 1
    },
    {
      question: "Which number is a palindrome?",
      options: ["4004", "4014", "4104", "4401"],
      answer: 0
    },
    {
      question: "For a suitable 4-digit Kaprekar routine, what operation is repeated?",
      options: [
        "Add ascending and descending numbers",
        "Subtract the smaller arrangement from the larger arrangement",
        "Multiply both arrangements",
        "Divide the larger by the smaller"
      ],
      answer: 1
    },
    {
      question: "Which statement about the Collatz conjecture is correct?",
      options: [
        "It has been proved for every positive integer",
        "It is an unsolved conjecture",
        "It applies only to even numbers",
        "It always reaches 6174"
      ],
      answer: 1
    },
    {
      question: "Why is estimation useful before an exact calculation?",
      options: [
        "It can provide a quick reasonableness check",
        "It guarantees the exact answer",
        "It removes the need to understand the problem",
        "It always gives a smaller value"
      ],
      answer: 0
    },
    {
      question: "If a pattern fits the first two terms but fails on later terms, what should you do?",
      options: [
        "Keep the same rule",
        "Recheck and find a rule that fits all relevant terms",
        "Ignore the later terms",
        "Choose the largest term"
      ],
      answer: 1
    },
    {
      question: "What is 3 × 3 + 1?",
      options: ["9", "10", "11", "12"],
      answer: 1
    },
    {
      question: "Starting from 10 in the Collatz process, what is the next term?",
      options: ["5", "20", "31", "4"],
      answer: 0
    },
    {
      question: "Which pair contains two palindromic numbers?",
      options: ["121 and 1331", "123 and 132", "245 and 542", "120 and 121"],
      answer: 0
    },
    {
      question: "What is the smallest number using 1, 5, 8 and 9 once each?",
      options: ["1589", "1859", "1598", "1958"],
      answer: 0
    },
    {
      question: "What is the largest number using 1, 5, 8 and 9 once each?",
      options: ["9851", "9815", "9581", "8951"],
      answer: 0
    }
  ],

  tooHard: [
    {
      question: "Apply one Kaprekar step to 3524. What is the result?",
      options: ["3087", "3088", "3097", "3187"],
      answer: 0
    },
    {
      question: "Apply one Kaprekar step to 2110.",
      options: ["0999", "0990", "1089", "999"],
      answer: 0
    },
    {
      question: "Starting with 13 in the Collatz process, what is the first next term?",
      options: ["26", "39", "40", "42"],
      answer: 1
    },
    {
      question: "Starting with 13, what is the second term after applying the Collatz rule twice?",
      options: ["20", "40", "41", "42"],
      answer: 1
    },
    {
      question: "Which statement best describes a number pattern?",
      options: [
        "It must always use addition",
        "It follows a rule connecting its terms",
        "It must contain only even numbers",
        "It must be increasing"
      ],
      answer: 1
    },
    {
      question: "If a 4-digit number has all four digits equal, why is the usual Kaprekar routine not a suitable test for the 6174 result?",
      options: [
        "There are not four distinct arrangements producing the required difference",
        "The number is too large",
        "All digits become negative",
        "Kaprekar uses only 3-digit numbers"
      ],
      answer: 0
    },
    {
      question: "Which is the best reason to verify an estimated answer?",
      options: [
        "To see whether the estimate is sensible",
        "To make the estimate exact",
        "To avoid understanding units",
        "To guarantee zero error"
      ],
      answer: 0
    },
    {
      question: "Which number is both a palindrome and a multiple of 11?",
      options: ["121", "123", "125", "127"],
      answer: 0
    },
    {
      question: "If the rule is 'add 3, then multiply by 2', starting from 4, what is the next term?",
      options: ["11", "14", "8", "7"],
      answer: 0
    },
    {
      question: "For the pattern 4, 7, 13, 25, the rule from one term to the next is:",
      options: [
        "Multiply by 2 and subtract 1",
        "Add 3 each time",
        "Multiply by 3",
        "Subtract 1 each time"
      ],
      answer: 0
    },
    {
      question: "What is the main mathematical skill developed by digit-rearrangement puzzles?",
      options: [
        "Understanding place value",
        "Memorising tables only",
        "Drawing circles",
        "Measuring temperature"
      ],
      answer: 0
    },
    {
      question: "Why is a clock-pattern puzzle different from an unrestricted digit puzzle?",
      options: [
        "Valid times must obey hour and minute limits",
        "Clocks cannot show repeated digits",
        "Clocks have no numbers",
        "All times are palindromes"
      ],
      answer: 0
    },
    {
      question: "What is the next Collatz term after 16?",
      options: ["8", "32", "49", "15"],
      answer: 0
    },
    {
      question: "What is the next Collatz term after 5?",
      options: ["10", "15", "16", "11"],
      answer: 2
    },
    {
      question: "Which strategy is strongest when solving an unfamiliar number puzzle?",
      options: [
        "Test small cases and identify a rule",
        "Guess the final answer",
        "Use only multiplication",
        "Ignore exceptions"
      ],
      answer: 0
    },
    {
      question: "A good mathematical rule should:",
      options: [
        "Explain the observed terms consistently",
        "Work for only one example",
        "Always increase numbers",
        "Never use subtraction"
      ],
      answer: 0
    },
    {
      question: "What does a palindrome demonstrate about its digits?",
      options: [
        "A form of left-right symmetry",
        "That all digits are even",
        "That the number is prime",
        "That the digits are increasing"
      ],
      answer: 0
    },
    {
      question: "Which is an example of estimation?",
      options: [
        "Saying a 198-page book has about 200 pages",
        "Counting exactly 198 pages",
        "Writing 198 as 189",
        "Ignoring the number of pages"
      ],
      answer: 0
    },
    {
      question: "Why should units be considered when estimating a quantity?",
      options: [
        "The unit gives meaning to the estimate",
        "Units always make estimates exact",
        "Units are unnecessary",
        "Units change every number to zero"
      ],
      answer: 0
    },
    {
      question: "Which process is algorithmic because it follows fixed steps?",
      options: [
        "Kaprekar's routine",
        "Random guessing",
        "Choosing any number",
        "Drawing a picture"
      ],
      answer: 0
    }
  ],

  extreme: [
    {
      question: "What is one Kaprekar step for 6174 itself?",
      options: ["6174", "6147", "6417", "6741"],
      answer: 0
    },
    {
      question: "Why does 6174 remain unchanged under the suitable 4-digit Kaprekar routine?",
      options: [
        "7641 − 1467 = 6174",
        "6174 − 6174 = 6174",
        "7164 − 1467 = 6174",
        "7641 + 1467 = 6174"
      ],
      answer: 0
    },
    {
      question: "Starting with 27, the Collatz sequence begins 27, 82, 41, ... What is the next term?",
      options: ["82", "123", "124", "83"],
      answer: 2
    },
    {
      question: "For the Collatz sequence starting at 10, how many steps are needed to reach 1?",
      options: ["4", "5", "6", "7"],
      answer: 2
    },
    {
      question: "If a number pattern is 2, 5, 11, 23, 47, what is the rule?",
      options: [
        "Multiply by 2 and add 1",
        "Add consecutive odd numbers",
        "Multiply by 3 and subtract 1",
        "Add 3 each time"
      ],
      answer: 0
    },
    {
      question: "Using digits 1, 2, 3 and 4 once each, what is the difference between the largest and smallest numbers?",
      options: ["3087", "3086", "3078", "3187"],
      answer: 0
    },
    {
      question: "Which number is the smallest 5-digit palindrome using 1, 2, 3, 4 and 5 as its first five digits in a symmetric construction?",
      options: ["12321", "12341", "12421", "12521"],
      answer: 0
    },
    {
      question: "What is the key idea behind a supercell puzzle?",
      options: [
        "Compare a cell with every required neighbour",
        "Choose the centre cell automatically",
        "Choose the largest number in the whole grid",
        "Choose every even number"
      ],
      answer: 0
    },
    {
      question: "Why is the Collatz statement called a conjecture?",
      options: [
        "It is believed to be true but has not been proved for all positive integers",
        "It is known to be false",
        "It applies only to decimals",
        "It was proved centuries ago"
      ],
      answer: 0
    },
    {
      question: "Which statement about estimation is strongest?",
      options: [
        "The best estimate depends on the purpose and available information",
        "Every estimate must be exact",
        "Estimation is useful only in mathematics exams",
        "A smaller estimate is always better"
      ],
      answer: 0
    },
    {
      question: "If a sequence follows the rule 'multiply by 3 and subtract 2', what comes after 8?",
      options: ["22", "24", "26", "28"],
      answer: 0
    },
    {
      question: "For the sequence 1, 4, 10, 22, 46, what is the rule?",
      options: [
        "Multiply by 2 and add 2",
        "Multiply by 2 and subtract 1",
        "Add 6 each time",
        "Multiply by 3 and add 1"
      ],
      answer: 0
    },
    {
      question: "Which property makes 1221 a palindrome?",
      options: [
        "The first and last digits match and the middle pair matches",
        "All digits are prime",
        "The number is even",
        "Its digits increase"
      ],
      answer: 0
    },
    {
      question: "Why is testing several terms important when finding a pattern rule?",
      options: [
        "Different rules can fit a small number of terms",
        "The first term is always wrong",
        "Patterns never have rules",
        "More terms make every answer exact"
      ],
      answer: 0
    },
    {
      question: "Which activity most directly combines place value and algorithmic thinking?",
      options: [
        "Kaprekar's routine",
        "Colouring a shape",
        "Reading a clock",
        "Counting objects"
      ],
      answer: 0
    },
    {
      question: "If an estimate is 500 and the actual value is 492, the estimate is:",
      options: [
        "Reasonably close",
        "Exactly correct",
        "Always wrong",
        "Impossible to compare"
      ],
      answer: 0
    },
    {
      question: "Which is a valid 4-digit number for studying the usual Kaprekar routine?",
      options: ["3524", "1111", "2222", "0000"],
      answer: 0
    },
    {
      question: "What is the main lesson of the number games in this chapter?",
      options: [
        "Simple rules can create rich mathematical patterns",
        "All mathematics is guessing",
        "Only calculators can solve number problems",
        "Patterns cannot be explained"
      ],
      answer: 0
    },
    {
      question: "Which statement best describes computational thinking?",
      options: [
        "Breaking a problem into logical, testable steps",
        "Doing calculations without a plan",
        "Memorising every possible answer",
        "Avoiding patterns"
      ],
      answer: 0
    },
    {
      question: "A mathematical puzzle is most convincing when the proposed rule:",
      options: [
        "Explains all the relevant observations and survives checking",
        "Works only once",
        "Produces the biggest answer",
        "Uses the most operations"
      ],
      answer: 0
    },
    {
      question: "What should you do if your first rule fails on a later term?",
      options: [
        "Reject or revise the rule and test a better one",
        "Ignore the later term",
        "Change the question",
        "Keep the rule without checking"
      ],
      answer: 0
    },
    {
      question: "Which pair contains two numbers with the same digits in reverse order?",
      options: ["123 and 321", "124 and 421", "135 and 351", "All of these"],
      answer: 3
    },
    {
      question: "What is the first result when applying the Collatz rule to 7?",
      options: ["14", "21", "22", "8"],
      answer: 2
    },
    {
      question: "What is the next result after 22 in the Collatz sequence?",
      options: ["11", "44", "67", "10"],
      answer: 0
    },
    {
      question: "Which statement is true about number patterns?",
      options: [
        "A pattern may use different operations if a definite rule explains the terms",
        "Every pattern must add the same number",
        "Every pattern must multiply by the same number",
        "Every pattern must increase"
      ],
      answer: 0
    }
  ]
};

/*==================================================
  CONCEPT QUIZZER
  PACKAGE 13.1.4 — QUIZ DATABASE ADDITION
  CLASS 6 • MATHEMATICS • CHAPTER 4
  Data Handling and Presentation
==================================================*/

quizDatabase.class6 = quizDatabase.class6 || {};
quizDatabase.class6.mathematics =
  quizDatabase.class6.mathematics || {};

quizDatabase.class6.mathematics["data-handling-and-presentation"] = {

  easy: [
    { question: "What is data?", options: ["Collected information", "A single answer", "Only a picture", "A colour"], answer: 0 },
    { question: "One complete group of tally marks represents:", options: ["3", "4", "5", "10"], answer: 2 },
    { question: "What does frequency tell us?", options: ["How often something occurs", "Its colour", "Its shape", "Its name"], answer: 0 },
    { question: "What is used to explain the value of a symbol in a pictograph?", options: ["Title", "Key", "Border", "Heading"], answer: 1 },
    { question: "Which graph uses rectangular bars for categories?", options: ["Bar graph", "Pictograph only", "Circle", "Number line"], answer: 0 },
    { question: "If one symbol represents 2 books, 5 symbols represent:", options: ["7", "10", "12", "15"], answer: 1 },
    { question: "If frequencies are 3, 4 and 5, the total is:", options: ["10", "11", "12", "13"], answer: 2 },
    { question: "What should you check before reading a graph?", options: ["The scale", "The phone battery", "The border", "The font"], answer: 0 },
    { question: "Which is an example of data?", options: ["Favourite sports of students", "A blank page", "A pencil", "A colour alone"], answer: 0 },
    { question: "A bar graph is mainly used to:", options: ["Compare categories", "Write stories", "Draw faces", "Measure time directly"], answer: 0 }
  ],

  medium: [
    { question: "A tally record has two groups of five and three extra marks. What is the frequency?", options: ["10", "12", "13", "15"], answer: 2 },
    { question: "A pictograph uses 1 symbol = 4 students. Six symbols represent:", options: ["20", "24", "26", "30"], answer: 1 },
    { question: "Frequencies are 8, 6, 11 and 5. How many observations are there?", options: ["25", "28", "30", "32"], answer: 2 },
    { question: "A graph has scale 5 per division. A bar reaches the sixth division. Its value is:", options: ["25", "30", "35", "40"], answer: 1 },
    { question: "Which category has the greatest frequency: A=12, B=17, C=9, D=15?", options: ["A", "B", "C", "D"], answer: 1 },
    { question: "If 23 students chose cricket and 16 chose football, how many more chose cricket?", options: ["5", "6", "7", "8"], answer: 2 },
    { question: "A table records 9 red, 4 blue and 7 green objects. How many objects are recorded?", options: ["18", "19", "20", "21"], answer: 2 },
    { question: "If one pictograph symbol represents 5 cars, 7 symbols represent:", options: ["30", "35", "40", "45"], answer: 1 },
    { question: "Which statement is correct?", options: ["The key can be ignored", "Scale helps read graph values", "Every symbol always means 1", "Bars need not have values"], answer: 1 },
    { question: "A category has frequency 18 and another has frequency 11. Their difference is:", options: ["5", "6", "7", "8"], answer: 2 }
  ],

  hard: [
    { question: "A survey has frequencies 12, 15, 9 and 14. What is the total?", options: ["48", "49", "50", "51"], answer: 2 },
    { question: "A pictograph has 8 symbols and 1 symbol = 6 students. How many students are represented?", options: ["42", "48", "54", "56"], answer: 1 },
    { question: "A bar graph uses a scale of 10. A bar reaches 7 divisions. What is its value?", options: ["60", "70", "80", "90"], answer: 1 },
    { question: "A class records 18, 23, 15 and 24 books read by four groups. Which group read the most?", options: ["18", "23", "15", "24"], answer: 3 },
    { question: "The total frequency is 40. Three categories have frequencies 9, 12 and 7. The fourth frequency is:", options: ["10", "11", "12", "13"], answer: 0 },
    { question: "A pictograph uses 1 symbol = 8 kg. A half-symbol represents:", options: ["2 kg", "4 kg", "6 kg", "8 kg"], answer: 1 },
    { question: "If A=16 and B=24 in a bar graph, B exceeds A by:", options: ["6", "7", "8", "9"], answer: 2 },
    { question: "Which total matches frequencies 6, 9, 4, 11 and 10?", options: ["38", "40", "41", "42"], answer: 1 },
    { question: "A graph scale labels 0, 5, 10, 15, 20. A bar reaches 15. The value is:", options: ["10", "12", "15", "20"], answer: 2 },
    { question: "If 5 symbols represent 35 objects, one symbol represents:", options: ["5", "6", "7", "8"], answer: 2 },
    { question: "A survey records 31 votes for A and 19 for B. What percentage is not needed to answer 'how many more'?", options: ["31", "19", "12", "50"], answer: 3 },
    { question: "Which is the best first step when interpreting a bar graph?", options: ["Read the scale", "Guess the tallest bar", "Ignore labels", "Count the pixels"], answer: 0 },
    { question: "A table has 72 observations divided among four categories. Three frequencies are 18, 21 and 16. The last is:", options: ["15", "16", "17", "18"], answer: 2 },
    { question: "A pictograph has 9 full symbols and 1 half-symbol. If one full symbol = 10, the total is:", options: ["90", "95", "100", "105"], answer: 1 },
    { question: "Why can a misleading scale be a problem?", options: ["It can give a false impression", "It makes bars rectangular", "It adds categories", "It removes data automatically"], answer: 0 }
  ],

  tooHard: [
    { question: "A survey has total 96 observations. Frequencies are 21, 18, 27 and x. Find x.", options: ["28", "29", "30", "31"], answer: 2 },
    { question: "A pictograph uses 1 symbol = 12 units. How many symbols are needed for 84 units?", options: ["6", "7", "8", "9"], answer: 1 },
    { question: "A bar graph scale is 4 per division. A bar is 9 divisions high. What is its value?", options: ["32", "34", "36", "40"], answer: 2 },
    { question: "Values are 18, 25, 31 and 26. What is the difference between the greatest and least?", options: ["11", "12", "13", "14"], answer: 2 },
    { question: "A frequency table has 5 categories with total 75. Four frequencies are 14, 17, 12 and 19. The fifth is:", options: ["11", "12", "13", "14"], answer: 0 },
    { question: "If 3 half-symbols each represent half of 10, together they represent:", options: ["10", "15", "20", "30"], answer: 1 },
    { question: "A graph has scale 2 per small division. A bar reaches 13 small divisions. What is its value?", options: ["24", "26", "28", "30"], answer: 1 },
    { question: "A category has 42 observations. Another has 28. The first is how many times the second?", options: ["1.25", "1.5", "2", "2.5"], answer: 1 },
    { question: "A pictograph shows 4, 6, 3 and 7 symbols with key 1 symbol = 5. Total objects are:", options: ["90", "95", "100", "105"], answer: 2 },
    { question: "A bar graph has values 14, 21, 35 and 28. What fraction of the total belongs to 35?", options: ["1/4", "1/3", "2/5", "1/2"], answer: 1 },
    { question: "If all frequencies in a table are doubled, the total number of observations becomes:", options: ["Half", "The same", "Double", "Four times"], answer: 2 },
    { question: "A graph scale changes from 1 per division to 5 per division. If the bar stays at 6 divisions, the value changes from 6 to:", options: ["11", "20", "30", "36"], answer: 2 },
    { question: "A table has 120 observations. The first three categories contain 25%, 20% and 15%. How many remain?", options: ["40", "48", "50", "52"], answer: 1 },
    { question: "A pictograph has 11 symbols and 1 symbol = 6. One category is shown with 4 symbols. Its value is:", options: ["18", "20", "24", "30"], answer: 2 },
    { question: "Which conclusion is safest from a bar graph?", options: ["The tallest bar represents the greatest value shown", "The tallest bar proves the category is best in every situation", "The graph predicts the future exactly", "The graph proves causation"], answer: 0 },
    { question: "Frequencies 8, 12, 15, 9 and 6 have mean frequency:", options: ["8", "9", "10", "11"], answer: 2 },
    { question: "A total of 90 students are shown. 36 chose A. What percentage of the total is A?", options: ["30%", "35%", "40%", "45%"], answer: 2 },
    { question: "If a bar value is 48 on a scale of 6 per division, the bar reaches:", options: ["6", "7", "8", "9"], answer: 2 },
    { question: "A pictograph has 7 full symbols and 2 half-symbols. With 1 symbol = 8, total is:", options: ["56", "60", "64", "72"], answer: 1 },
    { question: "If a table is missing one frequency, which information is most useful for finding it?", options: ["The total observations and all other frequencies", "The title only", "The colour of the table", "The number of rows only"], answer: 0 }
  ],

  extreme: [
    { question: "A data set has total 240. Four categories are 48, 36, 54 and 42. The fifth is:", options: ["50", "54", "60", "64"], answer: 2 },
    { question: "A pictograph key is 1 symbol = 12. A category has 7 symbols and another has 4 symbols. Their difference is:", options: ["24", "30", "36", "48"], answer: 2 },
    { question: "A graph scale is 7 per division. A bar reaches 12 divisions. Its value is:", options: ["77", "84", "91", "98"], answer: 1 },
    { question: "A data set contains 150 observations. Category A is 36, B is 42, C is 28 and D is 19. The remaining category is:", options: ["25", "26", "27", "28"], answer: 2 },
    { question: "A pictograph shows 9 full symbols and 3 half-symbols. If one symbol = 16, the total is:", options: ["144", "160", "168", "192"], answer: 2 },
    { question: "Five categories have values 18, 24, 30, 36 and 42. What is the difference between the total and the sum of the first three?", options: ["54", "60", "72", "78"], answer: 1 },
    { question: "A bar graph uses 3 units per division. Two bars are 11 and 17 divisions high. Their difference is:", options: ["15", "18", "21", "24"], answer: 1 },
    { question: "A total of 180 observations is divided in the ratio 2:3:4. The largest category is:", options: ["40", "60", "80", "90"], answer: 2 },
    { question: "A pictograph has 15 symbols for 120 items. The key is:", options: ["6 per symbol", "7 per symbol", "8 per symbol", "10 per symbol"], answer: 2 },
    { question: "Values are 12, 18, 24, 30 and 36. If every value is increased by 5, the new total increases by:", options: ["5", "10", "20", "25"], answer: 3 },
    { question: "A frequency table totals 200. Four categories are 35%, 25%, 15% and 10%. The last category is:", options: ["15", "20", "25", "30"], answer: 1 },
    { question: "A graph scale is 4 per division. A bar represents 52. How many divisions does it reach?", options: ["11", "12", "13", "14"], answer: 2 },
    { question: "A pictograph uses a half-symbol for 5 units. Three half-symbols and four full symbols represent:", options: ["45", "50", "55", "60"], answer: 2 },
    { question: "A category has 64 observations and another has 40. The first exceeds the second by what percentage of the second?", options: ["40%", "50%", "60%", "80%"], answer: 2 },
    { question: "A graph shows A=28, B=35, C=49, D=42. What is C as a fraction of the total?", options: ["1/4", "7/22", "7/22 approximately", "1/3"], answer: 2 },
    { question: "A data set has 8 categories with average frequency 15. What is the total number of observations?", options: ["100", "110", "120", "130"], answer: 2 },
    { question: "A pictograph key changes from 1 symbol = 5 to 1 symbol = 8. For 10 symbols, the represented value increases by:", options: ["20", "25", "30", "35"], answer: 2 },
    { question: "A graph has equal divisions labelled 0, 12, 24, 36, 48. A bar reaches the third division. Its value is:", options: ["24", "30", "36", "48"], answer: 2 },
    { question: "A survey has 250 observations. A=70, B=55, C=45, D=30. What percentage belongs to the remaining category?", options: ["15%", "20%", "25%", "30%"], answer: 1 },
    { question: "A category is represented by 18 symbols at 4 items per symbol. Another has 12 symbols. How many more items does the first represent?", options: ["20", "24", "30", "36"], answer: 1 },
    { question: "If all values in a frequency table are multiplied by 3, the total frequency is multiplied by:", options: ["1", "2", "3", "6"], answer: 2 },
    { question: "A bar graph uses a scale of 2.5 per division. A bar reaches 8 divisions. Its value is:", options: ["16", "18", "20", "22"], answer: 2 },
    { question: "A total of 360 is divided into categories of 20%, 30%, 25% and 15%. The remaining percentage is:", options: ["5%", "10%", "15%", "20%"], answer: 1 },
    { question: "The frequencies 16, 24, 32 and 40 are shown. The greatest frequency is what fraction of the total?", options: ["1/4", "5/14", "5/7", "2/5"], answer: 1 },
    { question: "A misleading graph starts its vertical axis at 90 instead of 0. What is the main risk?", options: ["It may exaggerate visual differences", "It always makes bars shorter", "It changes the collected data", "It removes the need for a scale"], answer: 0 }
  ]
};

"use strict";
quizDatabase.class6 = quizDatabase.class6 || {};
quizDatabase.class6.mathematics = quizDatabase.class6.mathematics || {};
quizDatabase.class6.mathematics["prime-time"] = {
  easy:[
    {question:"Which number is a factor of 12?",options:["5","6","7","11"],answer:1},
    {question:"Which is a multiple of 7?",options:["13","14","15","16"],answer:1},
    {question:"How many positive factors does a prime number have?",options:["1","2","3","Many"],answer:1},
    {question:"Which number is prime?",options:["9","12","13","15"],answer:2},
    {question:"Which number is composite?",options:["2","3","5","8"],answer:3},
    {question:"The number 1 is:",options:["Prime","Composite","Neither","Even prime"],answer:2},
    {question:"Which is divisible by 5?",options:["32","41","45","48"],answer:2},
    {question:"Which is divisible by 10?",options:["25","30","35","42"],answer:1},
    {question:"Which is an even prime number?",options:["1","2","4","6"],answer:1},
    {question:"Which is a factor of 20?",options:["3","4","7","9"],answer:1}
  ],
  medium:[
    {question:"Which list contains all positive factors of 18?",options:["1,2,3,6,9,18","1,3,6,18","2,3,9,18","1,2,4,9,18"],answer:0},
    {question:"Which number is a common factor of 12 and 20?",options:["3","4","5","7"],answer:1},
    {question:"Which is a common multiple of 4 and 6?",options:["10","12","14","18"],answer:1},
    {question:"What is the least positive common multiple of 4 and 6?",options:["8","10","12","24"],answer:2},
    {question:"Which number is divisible by 3?",options:["124","135","142","151"],answer:1},
    {question:"Which number is divisible by 9?",options:["126","134","145","152"],answer:0},
    {question:"Which number is divisible by 4?",options:["214","316","327","339"],answer:1},
    {question:"If 6 × 8 = 48, which statement is correct?",options:["6 is a multiple of 48","48 is a factor of 6","6 is a factor of 48","8 is a multiple of 48"],answer:2},
    {question:"Which number is NOT prime?",options:["17","19","21","23"],answer:2},
    {question:"How many positive factors does 16 have?",options:["3","4","5","6"],answer:2}
  ],
  hard:[
    {question:"What is the greatest common factor of 18 and 24?",options:["3","6","9","12"],answer:1},
    {question:"What is the least common multiple of 6 and 8?",options:["12","18","24","48"],answer:2},
    {question:"Which number is divisible by both 3 and 5?",options:["42","50","60","64"],answer:2},
    {question:"Which number is prime?",options:["27","29","33","39"],answer:1},
    {question:"How many positive factors does 24 have?",options:["6","7","8","9"],answer:2},
    {question:"Which pair has a common factor greater than 1?",options:["8 and 15","14 and 25","18 and 27","11 and 20"],answer:2},
    {question:"The digit sum of 729 is 18. Therefore 729 is divisible by:",options:["2 only","3 and 9","5 only","10 only"],answer:1},
    {question:"Which is the smallest common multiple of 5 and 8?",options:["20","30","40","80"],answer:2},
    {question:"Which statement about factors is true?",options:["Every factor is greater than the number","Factors divide exactly","Factors are always multiples","A number has infinitely many factors"],answer:1},
    {question:"Which number has exactly two positive factors?",options:["25","27","31","35"],answer:2},
    {question:"Which number is divisible by 4?",options:["312","314","318","322"],answer:0},
    {question:"Which number is both a multiple of 6 and 9?",options:["18","21","27","30"],answer:0},
    {question:"The common factors of 16 and 24 include:",options:["5","6","8","12"],answer:2},
    {question:"Which number is composite?",options:["37","41","43","45"],answer:3},
    {question:"If 7 is a factor of 56, then 56 is:",options:["a factor of 7","a multiple of 7","prime","not divisible by 7"],answer:1}
  ],
  tooHard:[
    {question:"Find the greatest common factor of 36 and 48.",options:["6","8","12","16"],answer:2},
    {question:"Find the least common multiple of 12 and 15.",options:["30","45","60","120"],answer:2},
    {question:"Which number has exactly 6 positive factors?",options:["12","13","16","17"],answer:0},
    {question:"Which number between 40 and 50 is prime?",options:["42","45","47","49"],answer:2},
    {question:"A number is divisible by 2 and 5. What must be true?",options:["It ends in 0","It ends in 5","It is odd","It is prime"],answer:0},
    {question:"The prime factorisation of 60 contains:",options:["2,3,5","2,4,5","3,4,5","2,3,6"],answer:0},
    {question:"Which is a common multiple of 8 and 12?",options:["20","24","28","36"],answer:1},
    {question:"What is the greatest common factor of 45 and 60?",options:["5","10","15","20"],answer:2},
    {question:"Which number is divisible by 9 but not by 2?",options:["126","135","144","162"],answer:1},
    {question:"A number has factors 1, 2, 4, 7, 14 and 28. The number is:",options:["14","28","42","56"],answer:1},
    {question:"Which pair has least common multiple 24?",options:["4 and 6","6 and 8","8 and 12","3 and 8"],answer:1},
    {question:"Which number is divisible by both 4 and 3?",options:["18","24","26","34"],answer:1},
    {question:"If a number has exactly two positive factors, it must be:",options:["Even","Odd","Prime","Composite"],answer:2},
    {question:"Which number is NOT a factor of 72?",options:["6","8","9","11"],answer:3},
    {question:"Which pair has greatest common factor 9?",options:["18 and 27","18 and 24","27 and 36","45 and 54"],answer:0},
    {question:"What is the least positive number divisible by 4, 6 and 8?",options:["12","24","36","48"],answer:1},
    {question:"A number divisible by 3 has digit sum:",options:["Always 3","A multiple of 3","Always 9","Always even"],answer:1},
    {question:"Which number is divisible by 4 and 5?",options:["20","25","30","35"],answer:0},
    {question:"Which number has prime factors 2, 3 and 5?",options:["20","30","45","50"],answer:1},
    {question:"If 15 is a factor of 90, which is also true?",options:["90 is a multiple of 15","15 is a multiple of 90","90 is prime","15 has no factors"],answer:0}
  ],
  extreme:[
    {question:"Find the least positive number divisible by 6, 8 and 9.",options:["36","48","72","144"],answer:2},
    {question:"Find the greatest common factor of 72 and 108.",options:["12","18","24","36"],answer:1},
    {question:"A number has exactly 3 positive factors. Which form can it have?",options:["p² where p is prime","2p where p is prime","p³ for every prime p","Any odd number"],answer:0},
    {question:"Which number between 80 and 100 is prime?",options:["87","91","97","99"],answer:2},
    {question:"A number is divisible by 2, 3 and 5. What is the smallest positive such number?",options:["15","20","30","60"],answer:2},
    {question:"The prime factorisation of 84 is:",options:["2×3×14","2²×3×7","3²×2×7","2×6×7"],answer:1},
    {question:"Which number has exactly 8 positive factors?",options:["24","30","36","49"],answer:0},
    {question:"If two numbers have GCF 6 and LCM 60, which pair could be them?",options:["12 and 30","18 and 24","6 and 30","10 and 36"],answer:0},
    {question:"The LCM of 18 and 24 is:",options:["48","54","72","96"],answer:2},
    {question:"Which number is divisible by 3, 4 and 5?",options:["40","50","60","70"],answer:2},
    {question:"If 2²×3×5 is a prime factorisation, the number is:",options:["30","40","60","90"],answer:2},
    {question:"Which pair has GCF 12?",options:["24 and 36","36 and 48","48 and 60","60 and 72"],answer:0},
    {question:"A number is divisible by 9 and 5 and lies between 40 and 60. It is:",options:["45","50","54","55"],answer:0},
    {question:"Which number has exactly 5 positive factors?",options:["16","18","24","30"],answer:0},
    {question:"The smallest common multiple of 9, 12 and 15 is:",options:["90","120","180","360"],answer:2},
    {question:"Which statement is always true for positive integers a and b?",options:["Every multiple of a is a factor of a","Every common multiple is divisible by each given number","Every factor is larger than its number","Every odd number is prime"],answer:1},
    {question:"Which number is divisible by 8?",options:["312","318","322","326"],answer:0},
    {question:"If a number is divisible by 6, it must be divisible by:",options:["2 and 3","3 and 5","4 and 6","5 and 6"],answer:0},
    {question:"Which number has prime factors only 2 and 3?",options:["42","48","50","70"],answer:1},
    {question:"If the LCM of two numbers is equal to their product, their GCF is:",options:["0","1","2","The larger number"],answer:1},
    {question:"Which number has exactly 10 positive factors?",options:["48","72","81","90"],answer:3},
    {question:"A number has prime factorisation 2³×3². What is the number?",options:["72","108","144","216"],answer:3},
    {question:"Which is the least positive common multiple of 14 and 21?",options:["28","35","42","84"],answer:2},
    {question:"Which number is both a perfect square and composite?",options:["13","17","25","29"],answer:2},
    {question:"Which number is divisible by 4, 6 and 9?",options:["36","48","54","72"],answer:3}
  ]

,
"perimeter-and-area": {
  easy: [
    {question:"What does perimeter measure?",options:["The surface inside a shape","The distance around a closed shape","The number of corners","The diagonal"],answer:1},
    {question:"What does area measure?",options:["Distance around a shape","Surface covered by a shape","Number of sides","Only the length"],answer:1},
    {question:"The perimeter of a square with side 5 cm is:",options:["10 cm","15 cm","20 cm","25 cm"],answer:2},
    {question:"The area of a square with side 4 cm is:",options:["8 cm²","12 cm²","16 cm²","20 cm²"],answer:2},
    {question:"The area of a rectangle 6 cm by 3 cm is:",options:["9 cm²","18 cm²","24 cm²","36 cm²"],answer:1},
    {question:"The perimeter of a rectangle 7 cm by 2 cm is:",options:["9 cm","14 cm","18 cm","21 cm"],answer:2},
    {question:"Which unit is suitable for perimeter?",options:["cm","cm²","m²","square cm"],answer:0},
    {question:"Which unit is suitable for area?",options:["cm","m","cm²","km"],answer:2},
    {question:"Fencing a garden usually requires finding its:",options:["Area","Perimeter","Volume","Mass"],answer:1},
    {question:"Tiling a floor usually requires finding its:",options:["Area","Perimeter","Height only","Boundary only"],answer:0}
  ],
  medium: [
    {question:"A rectangle is 12 m long and 5 m wide. Its perimeter is:",options:["17 m","24 m","34 m","60 m"],answer:2},
    {question:"A rectangle is 12 m long and 5 m wide. Its area is:",options:["17 m²","34 m²","60 m²","120 m²"],answer:2},
    {question:"A square has perimeter 32 cm. Its side is:",options:["6 cm","8 cm","10 cm","16 cm"],answer:1},
    {question:"A square has side 9 m. Its area is:",options:["18 m²","36 m²","72 m²","81 m²"],answer:3},
    {question:"A rectangle has length 10 cm and breadth 4 cm. The difference between its perimeter and area numerically is:",options:["10","12","14","16"],answer:2},
    {question:"A rectangle has perimeter 30 cm and length 10 cm. Its breadth is:",options:["3 cm","5 cm","10 cm","15 cm"],answer:1},
    {question:"A rectangle has area 48 cm² and length 8 cm. Its breadth is:",options:["4 cm","5 cm","6 cm","8 cm"],answer:2},
    {question:"Which is an area unit?",options:["m","m²","cm","km"],answer:1},
    {question:"A square side changes from 4 cm to 8 cm. Its area becomes:",options:["2 times","4 times","8 times","16 times"],answer:1},
    {question:"A shape has outside sides 5, 4, 3 and 6 cm. Its perimeter is:",options:["14 cm","16 cm","18 cm","20 cm"],answer:2}
  ],
  hard: [
    {question:"A rectangular field is 25 m by 14 m. How much fencing is needed for one complete boundary?",options:["39 m","50 m","78 m","350 m"],answer:2},
    {question:"A floor is 12 m by 8 m. Its area is:",options:["20 m²","40 m²","96 m²","192 m²"],answer:2},
    {question:"A square has area 121 cm². Its side is:",options:["9 cm","10 cm","11 cm","12 cm"],answer:2},
    {question:"A rectangle has perimeter 46 cm and length 15 cm. Its breadth is:",options:["6 cm","8 cm","10 cm","16 cm"],answer:1},
    {question:"A rectangle has area 72 cm² and breadth 8 cm. Its length is:",options:["7 cm","8 cm","9 cm","10 cm"],answer:2},
    {question:"A 10 cm by 6 cm rectangle has a 4 cm by 2 cm rectangle removed from a corner. Its remaining area is:",options:["44 cm²","48 cm²","52 cm²","60 cm²"],answer:2},
    {question:"Two rectangles of areas 24 cm² and 18 cm² are joined without overlap. Total area is:",options:["6 cm²","42 cm²","48 cm²","432 cm²"],answer:1},
    {question:"Which statement is correct?",options:["Perimeter uses square units","Area uses linear units","Perimeter measures boundary and area measures surface","Area and perimeter always have the same value"],answer:2},
    {question:"A square has side 12 cm. Its perimeter is:",options:["24 cm","36 cm","48 cm","144 cm"],answer:2},
    {question:"A rectangle has length 18 m and breadth 7 m. Its area is:",options:["50 m²","126 m²","140 m²","252 m²"],answer:1},
    {question:"A square's side is doubled. Its perimeter becomes:",options:["Half","Double","Triple","Four times"],answer:1},
    {question:"A square's side is doubled. Its area becomes:",options:["Double","Triple","Four times","Eight times"],answer:2},
    {question:"A rectangle is 20 cm by 9 cm. Its perimeter is:",options:["29 cm","40 cm","58 cm","180 cm"],answer:2},
    {question:"A 15 m by 10 m garden is fenced. If a 3 m gate is left unfenced, fencing needed is:",options:["47 m","50 m","53 m","56 m"],answer:0},
    {question:"A rectangular sheet has area 90 cm² and length 15 cm. Its breadth is:",options:["5 cm","6 cm","7 cm","8 cm"],answer:1}
  ],
  tooHard: [
    {question:"A rectangle has perimeter 64 cm. If its length is 20 cm, its breadth is:",options:["10 cm","12 cm","14 cm","16 cm"],answer:1},
    {question:"A square has perimeter 52 m. Its area is:",options:["104 m²","169 m²","196 m²","208 m²"],answer:1},
    {question:"A 18 m by 12 m rectangle has a 6 m by 4 m corner removed. Remaining area is:",options:["168 m²","180 m²","192 m²","200 m²"],answer:1},
    {question:"A rectangular garden is 30 m by 18 m. A path is made around its boundary. If only the outer boundary is fenced, the required fence length is:",options:["48 m","60 m","96 m","540 m"],answer:2},
    {question:"A rectangle has area 144 cm². Which pair could be its length and breadth?",options:["10 cm and 14 cm","12 cm and 12 cm","16 cm and 9 cm","18 cm and 7 cm"],answer:2},
    {question:"A square and rectangle have the same perimeter of 40 cm. The square has side 10 cm. Which rectangle dimensions are possible?",options:["5 cm × 15 cm","8 cm × 12 cm","9 cm × 11 cm","4 cm × 16 cm"],answer:1},
    {question:"A 12 cm × 10 cm rectangle is divided into two equal rectangles along its length. Each has area:",options:["30 cm²","50 cm²","60 cm²","120 cm²"],answer:2},
    {question:"A 20 cm × 8 cm rectangle is divided into four equal squares. What is the side of each square?",options:["2 cm","4 cm","5 cm","8 cm"],answer:2},
    {question:"A square has area 225 cm². Its perimeter is:",options:["45 cm","60 cm","75 cm","90 cm"],answer:1},
    {question:"A rectangular floor is 9 m × 7 m. Tiles cover 1 m² each. How many tiles are needed if there are no gaps?",options:["16","32","63","72"],answer:2},
    {question:"A 16 cm × 9 cm rectangle has the same area as a square. The square's side is:",options:["10 cm","12 cm","14 cm","15 cm"],answer:1},
    {question:"A rectangle's length is 3 times its breadth. If breadth is 6 cm, its perimeter is:",options:["18 cm","24 cm","36 cm","48 cm"],answer:2},
    {question:"A square and a rectangle both have area 64 cm². If rectangle length is 16 cm, its breadth is:",options:["2 cm","4 cm","8 cm","16 cm"],answer:1},
    {question:"A rectangular plot 24 m × 15 m has a 2 m wide gate opening. Fence needed is:",options:["74 m","76 m","78 m","80 m"],answer:0},
    {question:"A shape is made of two non-overlapping rectangles with areas 35 cm² and 42 cm². Its area is:",options:["7 cm²","70 cm²","77 cm²","1470 cm²"],answer:2},
    {question:"If every side of a polygon is measured and added, the result gives its:",options:["Area","Perimeter","Volume","Capacity"],answer:1},
    {question:"A rectangle has perimeter 50 cm. If breadth is 8 cm, length is:",options:["9 cm","12 cm","17 cm","25 cm"],answer:2},
    {question:"A square has side 15 cm. If a 5 cm × 5 cm square is cut from one corner, remaining area is:",options:["175 cm²","200 cm²","225 cm²","250 cm²"],answer:1},
    {question:"A rectangle's area is 120 m² and length is 15 m. Its perimeter is:",options:["30 m","38 m","46 m","60 m"],answer:2},
    {question:"A square has perimeter 80 cm. What is its area?",options:["160 cm²","320 cm²","400 cm²","640 cm²"],answer:2}
  ],
  extreme: [
    {question:"A rectangle has area 240 cm² and perimeter 64 cm. Its dimensions are:",options:["10 cm × 24 cm","12 cm × 20 cm","15 cm × 16 cm","8 cm × 30 cm"],answer:1},
    {question:"A square has the same area as a rectangle 18 cm × 8 cm. Its side is:",options:["10 cm","12 cm","14 cm","16 cm"],answer:1},
    {question:"A 30 m × 20 m rectangle has a 10 m × 8 m rectangular section removed. Remaining area is:",options:["480 m²","500 m²","520 m²","600 m²"],answer:2},
    {question:"A rectangle has perimeter 100 m. If its length is 30 m, its area is:",options:["400 m²","500 m²","600 m²","900 m²"],answer:2},
    {question:"A square has perimeter 96 cm. A rectangle has the same perimeter and length 32 cm. Which is its breadth?",options:["12 cm","16 cm","20 cm","24 cm"],answer:1},
    {question:"A 24 cm × 16 cm rectangle is divided into squares of greatest possible equal side length. What is that side length?",options:["4 cm","6 cm","8 cm","12 cm"],answer:2},
    {question:"A rectangular room is 8 m × 6 m. A 1 m wide strip runs along the entire inside boundary. What is the area left in the central rectangle?",options:["24 m²","30 m²","36 m²","48 m²"],answer:0},
    {question:"A square of side 20 cm has a 5 cm × 5 cm corner removed. The remaining area is:",options:["350 cm²","375 cm²","395 cm²","400 cm²"],answer:1},
    {question:"A rectangle and square have equal perimeter 48 cm. If rectangle dimensions are 16 cm × 8 cm, which has larger area?",options:["Rectangle by 16 cm²","Square by 16 cm²","Same area","Cannot determine"],answer:1},
    {question:"A rectangle's length is twice its breadth. If its area is 98 cm², its breadth is:",options:["5 cm","6 cm","7 cm","8 cm"],answer:2},
    {question:"A square's area is 196 m². What is its perimeter?",options:["28 m","42 m","56 m","196 m"],answer:2},
    {question:"A rectangular field is 45 m × 28 m. A 3 m gate is left open. Fence required is:",options:["143 m","146 m","149 m","152 m"],answer:0},
    {question:"A 20 cm × 15 cm rectangle is split into two rectangles of equal area. If one part has length 10 cm, its breadth is:",options:["10 cm","15 cm","20 cm","30 cm"],answer:1},
    {question:"A square and rectangle have equal area 144 cm². The rectangle is 18 cm long. Its perimeter is:",options:["34 cm","36 cm","52 cm","68 cm"],answer:2},
    {question:"A 25 m × 16 m garden is surrounded by a 1 m wide path outside it. The outer rectangle is 27 m × 18 m. Its area is:",options:["400 m²","450 m²","486 m²","550 m²"],answer:2},
    {question:"A rectangle has dimensions 14 cm × 9 cm. If both dimensions increase by 2 cm, the new area is:",options:["126 cm²","150 cm²","176 cm²","196 cm²"],answer:2},
    {question:"A square of side 10 cm is enlarged to side 15 cm. By how much does its area increase?",options:["50 cm²","100 cm²","125 cm²","150 cm²"],answer:2},
    {question:"A rectangle has area 180 cm² and length 20 cm. If its length is reduced by 5 cm while area remains 180 cm², new breadth is:",options:["8 cm","10 cm","12 cm","15 cm"],answer:2},
    {question:"A rectangular sheet 32 cm × 20 cm is cut into the largest possible equal squares. How many squares are obtained?",options:["8","10","16","20"],answer:1},
    {question:"A square and rectangle have equal area 100 cm². The rectangle is 25 cm × 4 cm. Which has smaller perimeter?",options:["Square","Rectangle","Same","Cannot determine"],answer:0},
    {question:"A rectangle has perimeter 72 cm. If its length and breadth differ by 10 cm, the dimensions are:",options:["23 cm and 13 cm","21 cm and 15 cm","26 cm and 10 cm","30 cm and 6 cm"],answer:0},
    {question:"A 12 m × 9 m room is covered by square tiles of side 3 m. How many complete tiles are needed?",options:["9","12","18","36"],answer:1},
    {question:"A rectangular field 40 m × 25 m has a 5 m × 5 m pond inside it. Land area remaining is:",options:["975 m²","1000 m²","1025 m²","1125 m²"],answer:0},
    {question:"A square has perimeter equal to the perimeter of a 18 cm × 6 cm rectangle. What is the square's area?",options:["144 cm²","196 cm²","225 cm²","256 cm²"],answer:2},
    {question:"A rectangle has area 300 cm² and one side 20 cm. If both sides are increased by 5 cm, new area is:",options:["375 cm²","400 cm²","450 cm²","500 cm²"],answer:2}
]
}

};
/*==========================================================
  PACKAGE 13.1.7 — FIXED CHAPTER BINDINGS
  Chapters 6 and 7
==========================================================*/
quizDatabase.class6 = quizDatabase.class6 || {};
quizDatabase.class6.mathematics =
  quizDatabase.class6.mathematics || {};

quizDatabase.class6.mathematics["perimeter-and-area"] = {
  easy: [
    {question:"What does perimeter measure?",options:["The surface inside a shape","The distance around a closed shape","The number of corners","The diagonal"],answer:1},
    {question:"What does area measure?",options:["Distance around a shape","Surface covered by a shape","Number of sides","Only the length"],answer:1},
    {question:"The perimeter of a square with side 5 cm is:",options:["10 cm","15 cm","20 cm","25 cm"],answer:2},
    {question:"The area of a square with side 4 cm is:",options:["8 cm²","12 cm²","16 cm²","20 cm²"],answer:2},
    {question:"The area of a rectangle 6 cm by 3 cm is:",options:["9 cm²","18 cm²","24 cm²","36 cm²"],answer:1},
    {question:"The perimeter of a rectangle 7 cm by 2 cm is:",options:["9 cm","14 cm","18 cm","21 cm"],answer:2},
    {question:"Which unit is suitable for perimeter?",options:["cm","cm²","m²","square cm"],answer:0},
    {question:"Which unit is suitable for area?",options:["cm","m","cm²","km"],answer:2},
    {question:"Fencing a garden usually requires finding its:",options:["Area","Perimeter","Volume","Mass"],answer:1},
    {question:"Tiling a floor usually requires finding its:",options:["Area","Perimeter","Height only","Boundary only"],answer:0}
  ],
  medium: [
    {question:"A rectangle is 12 m long and 5 m wide. Its perimeter is:",options:["17 m","24 m","34 m","60 m"],answer:2},
    {question:"A rectangle is 12 m long and 5 m wide. Its area is:",options:["17 m²","34 m²","60 m²","120 m²"],answer:2},
    {question:"A square has perimeter 32 cm. Its side is:",options:["6 cm","8 cm","10 cm","16 cm"],answer:1},
    {question:"A square has side 9 m. Its area is:",options:["18 m²","36 m²","72 m²","81 m²"],answer:3},
    {question:"A rectangle has length 10 cm and breadth 4 cm. The difference between its perimeter and area numerically is:",options:["10","12","14","16"],answer:2},
    {question:"A rectangle has perimeter 30 cm and length 10 cm. Its breadth is:",options:["3 cm","5 cm","10 cm","15 cm"],answer:1},
    {question:"A rectangle has area 48 cm² and length 8 cm. Its breadth is:",options:["4 cm","5 cm","6 cm","8 cm"],answer:2},
    {question:"Which is an area unit?",options:["m","m²","cm","km"],answer:1},
    {question:"A square side changes from 4 cm to 8 cm. Its area becomes:",options:["2 times","4 times","8 times","16 times"],answer:1},
    {question:"A shape has outside sides 5, 4, 3 and 6 cm. Its perimeter is:",options:["14 cm","16 cm","18 cm","20 cm"],answer:2}
  ],
  hard: [
    {question:"A rectangular field is 25 m by 14 m. How much fencing is needed for one complete boundary?",options:["39 m","50 m","78 m","350 m"],answer:2},
    {question:"A floor is 12 m by 8 m. Its area is:",options:["20 m²","40 m²","96 m²","192 m²"],answer:2},
    {question:"A square has area 121 cm². Its side is:",options:["9 cm","10 cm","11 cm","12 cm"],answer:2},
    {question:"A rectangle has perimeter 46 cm and length 15 cm. Its breadth is:",options:["6 cm","8 cm","10 cm","16 cm"],answer:1},
    {question:"A rectangle has area 72 cm² and breadth 8 cm. Its length is:",options:["7 cm","8 cm","9 cm","10 cm"],answer:2},
    {question:"A 10 cm by 6 cm rectangle has a 4 cm by 2 cm rectangle removed from a corner. Its remaining area is:",options:["44 cm²","48 cm²","52 cm²","60 cm²"],answer:2},
    {question:"Two rectangles of areas 24 cm² and 18 cm² are joined without overlap. Total area is:",options:["6 cm²","42 cm²","48 cm²","432 cm²"],answer:1},
    {question:"Which statement is correct?",options:["Perimeter uses square units","Area uses linear units","Perimeter measures boundary and area measures surface","Area and perimeter always have the same value"],answer:2},
    {question:"A square has side 12 cm. Its perimeter is:",options:["24 cm","36 cm","48 cm","144 cm"],answer:2},
    {question:"A rectangle has length 18 m and breadth 7 m. Its area is:",options:["50 m²","126 m²","140 m²","252 m²"],answer:1},
    {question:"A square's side is doubled. Its perimeter becomes:",options:["Half","Double","Triple","Four times"],answer:1},
    {question:"A square's side is doubled. Its area becomes:",options:["Double","Triple","Four times","Eight times"],answer:2},
    {question:"A rectangle is 20 cm by 9 cm. Its perimeter is:",options:["29 cm","40 cm","58 cm","180 cm"],answer:2},
    {question:"A 15 m by 10 m garden is fenced. If a 3 m gate is left unfenced, fencing needed is:",options:["47 m","50 m","53 m","56 m"],answer:0},
    {question:"A rectangular sheet has area 90 cm² and length 15 cm. Its breadth is:",options:["5 cm","6 cm","7 cm","8 cm"],answer:1}
  ],
  tooHard: [
    {question:"A rectangle has perimeter 64 cm. If its length is 20 cm, its breadth is:",options:["10 cm","12 cm","14 cm","16 cm"],answer:1},
    {question:"A square has perimeter 52 m. Its area is:",options:["104 m²","169 m²","196 m²","208 m²"],answer:1},
    {question:"A 18 m by 12 m rectangle has a 6 m by 4 m corner removed. Remaining area is:",options:["168 m²","180 m²","192 m²","200 m²"],answer:1},
    {question:"A rectangular garden is 30 m by 18 m. A path is made around its boundary. If only the outer boundary is fenced, the required fence length is:",options:["48 m","60 m","96 m","540 m"],answer:2},
    {question:"A rectangle has area 144 cm². Which pair could be its length and breadth?",options:["10 cm and 14 cm","12 cm and 12 cm","16 cm and 9 cm","18 cm and 7 cm"],answer:2},
    {question:"A square and rectangle have the same perimeter of 40 cm. The square has side 10 cm. Which rectangle dimensions are possible?",options:["5 cm × 15 cm","8 cm × 12 cm","9 cm × 11 cm","4 cm × 16 cm"],answer:1},
    {question:"A 12 cm × 10 cm rectangle is divided into two equal rectangles along its length. Each has area:",options:["30 cm²","50 cm²","60 cm²","120 cm²"],answer:2},
    {question:"A 20 cm × 8 cm rectangle is divided into four equal squares. What is the side of each square?",options:["2 cm","4 cm","5 cm","8 cm"],answer:2},
    {question:"A square has area 225 cm². Its perimeter is:",options:["45 cm","60 cm","75 cm","90 cm"],answer:1},
    {question:"A rectangular floor is 9 m × 7 m. Tiles cover 1 m² each. How many tiles are needed if there are no gaps?",options:["16","32","63","72"],answer:2},
    {question:"A 16 cm × 9 cm rectangle has the same area as a square. The square's side is:",options:["10 cm","12 cm","14 cm","15 cm"],answer:1},
    {question:"A rectangle's length is 3 times its breadth. If breadth is 6 cm, its perimeter is:",options:["18 cm","24 cm","36 cm","48 cm"],answer:2},
    {question:"A square and a rectangle both have area 64 cm². If rectangle length is 16 cm, its breadth is:",options:["2 cm","4 cm","8 cm","16 cm"],answer:1},
    {question:"A rectangular plot 24 m × 15 m has a 2 m wide gate opening. Fence needed is:",options:["74 m","76 m","78 m","80 m"],answer:0},
    {question:"A shape is made of two non-overlapping rectangles with areas 35 cm² and 42 cm². Its area is:",options:["7 cm²","70 cm²","77 cm²","1470 cm²"],answer:2},
    {question:"If every side of a polygon is measured and added, the result gives its:",options:["Area","Perimeter","Volume","Capacity"],answer:1},
    {question:"A rectangle has perimeter 50 cm. If breadth is 8 cm, length is:",options:["9 cm","12 cm","17 cm","25 cm"],answer:2},
    {question:"A square has side 15 cm. If a 5 cm × 5 cm square is cut from one corner, remaining area is:",options:["175 cm²","200 cm²","225 cm²","250 cm²"],answer:1},
    {question:"A rectangle's area is 120 m² and length is 15 m. Its perimeter is:",options:["30 m","38 m","46 m","60 m"],answer:2},
    {question:"A square has perimeter 80 cm. What is its area?",options:["160 cm²","320 cm²","400 cm²","640 cm²"],answer:2}
  ],
  extreme: [
    {question:"A rectangle has area 240 cm² and perimeter 64 cm. Its dimensions are:",options:["10 cm × 24 cm","12 cm × 20 cm","15 cm × 16 cm","8 cm × 30 cm"],answer:1},
    {question:"A square has the same area as a rectangle 18 cm × 8 cm. Its side is:",options:["10 cm","12 cm","14 cm","16 cm"],answer:1},
    {question:"A 30 m × 20 m rectangle has a 10 m × 8 m rectangular section removed. Remaining area is:",options:["480 m²","500 m²","520 m²","600 m²"],answer:2},
    {question:"A rectangle has perimeter 100 m. If its length is 30 m, its area is:",options:["400 m²","500 m²","600 m²","900 m²"],answer:2},
    {question:"A square has perimeter 96 cm. A rectangle has the same perimeter and length 32 cm. Which is its breadth?",options:["12 cm","16 cm","20 cm","24 cm"],answer:1},
    {question:"A 24 cm × 16 cm rectangle is divided into squares of greatest possible equal side length. What is that side length?",options:["4 cm","6 cm","8 cm","12 cm"],answer:2},
    {question:"A rectangular room is 8 m × 6 m. A 1 m wide strip runs along the entire inside boundary. What is the area left in the central rectangle?",options:["24 m²","30 m²","36 m²","48 m²"],answer:0},
    {question:"A square of side 20 cm has a 5 cm × 5 cm corner removed. The remaining area is:",options:["350 cm²","375 cm²","395 cm²","400 cm²"],answer:1},
    {question:"A rectangle and square have equal perimeter 48 cm. If rectangle dimensions are 16 cm × 8 cm, which has larger area?",options:["Rectangle by 16 cm²","Square by 16 cm²","Same area","Cannot determine"],answer:1},
    {question:"A rectangle's length is twice its breadth. If its area is 98 cm², its breadth is:",options:["5 cm","6 cm","7 cm","8 cm"],answer:2},
    {question:"A square's area is 196 m². What is its perimeter?",options:["28 m","42 m","56 m","196 m"],answer:2},
    {question:"A rectangular field is 45 m × 28 m. A 3 m gate is left open. Fence required is:",options:["143 m","146 m","149 m","152 m"],answer:0},
    {question:"A 20 cm × 15 cm rectangle is split into two rectangles of equal area. If one part has length 10 cm, its breadth is:",options:["10 cm","15 cm","20 cm","30 cm"],answer:1},
    {question:"A square and rectangle have equal area 144 cm². The rectangle is 18 cm long. Its perimeter is:",options:["34 cm","36 cm","52 cm","68 cm"],answer:2},
    {question:"A 25 m × 16 m garden is surrounded by a 1 m wide path outside it. The outer rectangle is 27 m × 18 m. Its area is:",options:["400 m²","450 m²","486 m²","550 m²"],answer:2},
    {question:"A rectangle has dimensions 14 cm × 9 cm. If both dimensions increase by 2 cm, the new area is:",options:["126 cm²","150 cm²","176 cm²","196 cm²"],answer:2},
    {question:"A square of side 10 cm is enlarged to side 15 cm. By how much does its area increase?",options:["50 cm²","100 cm²","125 cm²","150 cm²"],answer:2},
    {question:"A rectangle has area 180 cm² and length 20 cm. If its length is reduced by 5 cm while area remains 180 cm², new breadth is:",options:["8 cm","10 cm","12 cm","15 cm"],answer:2},
    {question:"A rectangular sheet 32 cm × 20 cm is cut into the largest possible equal squares. How many squares are obtained?",options:["8","10","16","20"],answer:1},
    {question:"A square and rectangle have equal area 100 cm². The rectangle is 25 cm × 4 cm. Which has smaller perimeter?",options:["Square","Rectangle","Same","Cannot determine"],answer:0},
    {question:"A rectangle has perimeter 72 cm. If its length and breadth differ by 10 cm, the dimensions are:",options:["23 cm and 13 cm","21 cm and 15 cm","26 cm and 10 cm","30 cm and 6 cm"],answer:0},
    {question:"A 12 m × 9 m room is covered by square tiles of side 3 m. How many complete tiles are needed?",options:["9","12","18","36"],answer:1},
    {question:"A rectangular field 40 m × 25 m has a 5 m × 5 m pond inside it. Land area remaining is:",options:["975 m²","1000 m²","1025 m²","1125 m²"],answer:0},
    {question:"A square has perimeter equal to the perimeter of a 18 cm × 6 cm rectangle. What is the square's area?",options:["144 cm²","196 cm²","225 cm²","256 cm²"],answer:2},
    {question:"A rectangle has area 300 cm² and one side 20 cm. If both sides are increased by 5 cm, new area is:",options:["375 cm²","400 cm²","450 cm²","500 cm²"],answer:2}
]
};
quizDatabase.class6.mathematics["fractions"] = {
  easy: [
    {question:"In 3/7, which number is the numerator?",options:["3","7","10","1"],answer:0},
    {question:"In 5/9, which number is the denominator?",options:["5","9","14","4"],answer:1},
    {question:"Which is a proper fraction?",options:["7/4","9/9","3/8","11/5"],answer:2},
    {question:"Which is an improper fraction?",options:["2/7","4/9","8/5","1/6"],answer:2},
    {question:"Which fraction is equivalent to 1/2?",options:["2/3","2/4","3/5","4/7"],answer:1},
    {question:"What is 2/7 + 3/7?",options:["5/14","5/7","6/7","1/7"],answer:1},
    {question:"What is 6/9 in simplest form?",options:["2/3","3/4","1/2","4/6"],answer:0},
    {question:"Which fraction is greater?",options:["2/7","5/7","3/7","1/7"],answer:1},
    {question:"Which is a mixed number?",options:["7/3","2 1/4","5/5","9/2"],answer:1},
    {question:"What is 1/4 of 20?",options:["4","5","6","8"],answer:1}
  ],
  medium: [
    {question:"Which fraction is equivalent to 3/5?",options:["6/10","5/8","9/20","12/25"],answer:0},
    {question:"Which is greater?",options:["3/8","3/5","3/10","3/12"],answer:1},
    {question:"What is 4/9 + 2/9?",options:["6/18","6/9","2/9","8/9"],answer:1},
    {question:"What is 7/10 − 3/10?",options:["4/10","4/20","10/4","3/10"],answer:0},
    {question:"Convert 2 1/3 to an improper fraction.",options:["5/3","6/3","7/3","8/3"],answer:2},
    {question:"Convert 11/4 to a mixed number.",options:["2 1/4","2 3/4","3 1/4","3 3/4"],answer:1},
    {question:"Which is greater: 2/3 or 3/5?",options:["2/3","3/5","They are equal","Cannot compare"],answer:0},
    {question:"What is 1/2 + 1/4?",options:["2/6","3/4","1/6","2/4"],answer:1},
    {question:"What is 3/4 − 1/4?",options:["1/2","2/8","3/8","1/4"],answer:0},
    {question:"What is 3/5 of 20?",options:["8","10","12","15"],answer:2}
  ],
  hard: [
    {question:"Which is the greatest fraction?",options:["5/8","2/3","7/12","3/5"],answer:1},
    {question:"What is 2/3 + 1/6?",options:["3/9","4/6","5/6","1"],answer:2},
    {question:"What is 5/6 − 1/4?",options:["1/2","7/12","2/3","3/8"],answer:1},
    {question:"Simplify 18/24.",options:["2/3","3/4","4/5","6/8"],answer:1},
    {question:"Which is equivalent to 7/9?",options:["14/18","21/30","28/45","35/54"],answer:0},
    {question:"Arrange from smallest to largest: 1/2, 3/4, 2/3.",options:["1/2, 2/3, 3/4","2/3, 1/2, 3/4","3/4, 2/3, 1/2","1/2, 3/4, 2/3"],answer:0},
    {question:"What is 3/8 + 1/4?",options:["4/12","5/8","7/8","1/2"],answer:1},
    {question:"What is 7/8 − 1/2?",options:["3/8","5/8","1/4","4/8"],answer:0},
    {question:"What is 2/5 of 35?",options:["10","12","14","15"],answer:2},
    {question:"Which fraction is closest to 1?",options:["2/5","5/6","3/8","4/9"],answer:1},
    {question:"Convert 3 2/5 to an improper fraction.",options:["15/5","16/5","17/5","18/5"],answer:2},
    {question:"Convert 17/6 to a mixed number.",options:["2 5/6","3 1/6","2 1/6","3 5/6"],answer:0},
    {question:"What is 1/3 + 1/2?",options:["2/5","3/5","5/6","1/6"],answer:2},
    {question:"What is 5/8 − 1/4?",options:["1/8","2/8","3/8","4/8"],answer:2},
    {question:"Which pair is equivalent?",options:["2/3 and 8/12","3/4 and 6/10","4/5 and 8/15","5/6 and 10/18"],answer:0}
  ],
  tooHard: [
    {question:"What is 5/6 + 7/12?",options:["12/18","17/12","19/12","1 1/12"],answer:1},
    {question:"What is 7/8 − 5/12?",options:["11/24","13/24","15/24","17/24"],answer:1},
    {question:"Simplify 42/56.",options:["2/3","3/4","4/5","6/7"],answer:1},
    {question:"Which fraction lies between 1/2 and 3/4?",options:["1/3","5/8","7/8","2/5"],answer:1},
    {question:"What is 3/4 of 28?",options:["18","20","21","24"],answer:2},
    {question:"A ribbon is 5/6 m long. If 1/3 m is used, how much remains?",options:["1/2 m","2/3 m","1/3 m","5/9 m"],answer:0},
    {question:"Which is greater?",options:["7/10","2/3","5/8","3/5"],answer:0},
    {question:"What is 2 1/4 + 1 2/3?",options:["3 5/7","3 11/12","4 1/12","3 7/12"],answer:1},
    {question:"What is 4 1/5 − 2 3/10?",options:["1 7/10","1 9/10","2 1/10","2 7/10"],answer:1},
    {question:"If 3/5 of a number is 18, the number is:",options:["24","30","36","45"],answer:1},
    {question:"Which fraction is in simplest form?",options:["12/18","15/25","14/21","11/17"],answer:3},
    {question:"What is 7/9 + 5/6?",options:["29/18","31/18","35/18","37/18"],answer:1},
    {question:"What is 11/12 − 2/9?",options:["25/36","29/36","31/36","33/36"],answer:1},
    {question:"Arrange 3/5, 7/10 and 2/3 from greatest to smallest.",options:["7/10, 2/3, 3/5","2/3, 7/10, 3/5","3/5, 2/3, 7/10","7/10, 3/5, 2/3"],answer:0},
    {question:"A tank is 3/4 full. After 1/6 of the tank is used, what fraction remains?",options:["5/12","7/12","2/3","1/2"],answer:1},
    {question:"Which fraction equals 2 3/5?",options:["10/5","12/5","13/5","15/5"],answer:2},
    {question:"What is 5/7 of 49?",options:["28","30","35","42"],answer:2},
    {question:"A class has 32 students. 3/8 are in a group. How many students are in the group?",options:["8","10","12","14"],answer:2},
    {question:"What is the difference between 5/6 and 3/8?",options:["7/24","9/24","11/24","13/24"],answer:2},
    {question:"Which common denominator is useful for 2/9 and 5/12?",options:["18","24","36","48"],answer:2}
  ],
  extreme: [
    {question:"What is 7/8 + 11/12?",options:["37/24","39/24","41/24","43/24"],answer:2},
    {question:"What is 13/15 − 7/18?",options:["19/90","29/90","31/90","41/90"],answer:2},
    {question:"Which fraction is exactly halfway between 1/3 and 2/3?",options:["1/2","2/5","3/5","5/9"],answer:0},
    {question:"A number is such that 4/7 of it is 32. What is the number?",options:["48","52","56","64"],answer:2},
    {question:"A 12 m rope has 5/8 of its length used. How much remains?",options:["4 m","4 1/2 m","4 1/4 m","5 m"],answer:0},
    {question:"Which is the greatest?",options:["11/15","7/10","5/6","13/18"],answer:2},
    {question:"Simplify 84/126.",options:["2/3","3/4","4/5","5/7"],answer:0},
    {question:"What is 2 5/6 + 3 3/4?",options:["5 7/12","6 1/12","6 7/12","5 11/12"],answer:2},
    {question:"What is 6 1/5 − 2 7/10?",options:["3 1/2","3 2/5","3 1/10","4 1/2"],answer:0},
    {question:"If 5/8 of a quantity is 45, the quantity is:",options:["56","64","72","80"],answer:2},
    {question:"A student reads 2/5 of a book on Monday and 1/4 on Tuesday. What fraction is left?",options:["7/20","9/20","11/20","13/20"],answer:1},
    {question:"Which is the smallest?",options:["7/12","5/8","2/3","3/5"],answer:0},
    {question:"A recipe needs 3/4 cup flour. It is multiplied by 2 1/2. How much flour is needed?",options:["1 1/2 cups","1 7/8 cups","2 cups","2 1/4 cups"],answer:1},
    {question:"What is 5/6 of 3/4?",options:["5/8","3/5","7/10","15/24"],answer:0},
    {question:"A journey is 48 km. 5/12 is completed. How many kilometres remain?",options:["20 km","24 km","28 km","32 km"],answer:2},
    {question:"What fraction of 60 is 45?",options:["1/2","2/3","3/4","4/5"],answer:2},
    {question:"If a fraction is multiplied by 1, its value:",options:["Doubles","Halves","Stays the same","Becomes zero"],answer:2},
    {question:"Which mixed number equals 29/8?",options:["3 1/8","3 5/8","3 7/8","4 1/8"],answer:1},
    {question:"A class spends 3/10 of its time on Maths and 1/4 on Science. What fraction is spent on both?",options:["7/20","11/20","13/20","17/20"],answer:0},
    {question:"Which fraction is equal to 1?",options:["7/8","9/10","11/11","12/13"],answer:2},
    {question:"A cake has 12 equal pieces. 5 are eaten in the morning and 3 in the evening. What fraction remains?",options:["1/3","1/4","1/2","4/12"],answer:2},
    {question:"Which fraction is greater than 3/4 but less than 1?",options:["2/3","7/8","1/2","3/5"],answer:1},
    {question:"What is 1 − 7/18?",options:["9/18","10/18","11/18","12/18"],answer:2},
    {question:"Which is equal to 4 3/5?",options:["20/5","21/5","23/5","25/5"],answer:2},
    {question:"A 60 km trip has 3/5 completed. How many km remain?",options:["18","20","24","36"],answer:1}
  ]
};


/* ==================================================
   CLASS 6 • MATHEMATICS • CHAPTER 8
   Playing with Constructions
   Package 13.1.8
================================================== */
quizDatabase.class6.mathematics["playing-with-constructions"] = {
  "easy": [
    {
      "question": "Which tool is mainly used to draw a circle?",
      "options": [
        "Ruler",
        "Compass",
        "Protractor",
        "Set square"
      ],
      "answer": 1
    },
    {
      "question": "The fixed point of a circle is called its:",
      "options": [
        "Radius",
        "Diameter",
        "Centre",
        "Vertex"
      ],
      "answer": 2
    },
    {
      "question": "A line segment from the centre to the circle is a:",
      "options": [
        "Diagonal",
        "Radius",
        "Diameter",
        "Chord"
      ],
      "answer": 1
    },
    {
      "question": "A diameter passes through the:",
      "options": [
        "Corner",
        "Centre",
        "Arc",
        "Radius only"
      ],
      "answer": 1
    },
    {
      "question": "How many right angles does a square have?",
      "options": [
        "2",
        "3",
        "4",
        "5"
      ],
      "answer": 2
    },
    {
      "question": "How many diagonals does a rectangle have?",
      "options": [
        "1",
        "2",
        "3",
        "4"
      ],
      "answer": 1
    },
    {
      "question": "Perpendicular lines meet at:",
      "options": [
        "45°",
        "60°",
        "90°",
        "180°"
      ],
      "answer": 2
    },
    {
      "question": "Which tool can transfer a fixed length?",
      "options": [
        "Compass",
        "Protractor",
        "Eraser",
        "Calculator"
      ],
      "answer": 0
    },
    {
      "question": "Which shape has four equal sides and four right angles?",
      "options": [
        "Rectangle",
        "Triangle",
        "Square",
        "Circle"
      ],
      "answer": 2
    },
    {
      "question": "A rectangle has equal:",
      "options": [
        "All four sides always",
        "Opposite sides",
        "Only diagonals never",
        "No sides"
      ],
      "answer": 1
    }
  ],
  "medium": [
    {
      "question": "A circle has radius 5 cm. What is its diameter?",
      "options": [
        "5 cm",
        "10 cm",
        "15 cm",
        "25 cm"
      ],
      "answer": 1
    },
    {
      "question": "Which instrument is best for drawing a straight 7 cm segment?",
      "options": [
        "Compass",
        "Ruler",
        "Protractor",
        "Divider only"
      ],
      "answer": 1
    },
    {
      "question": "What happens to the compass opening when transferring a length?",
      "options": [
        "It should remain unchanged",
        "It should double",
        "It should become zero",
        "It should be random"
      ],
      "answer": 0
    },
    {
      "question": "Which statement about a square is true?",
      "options": [
        "Only opposite sides are equal",
        "All sides are equal",
        "No angle is 90°",
        "It has three diagonals"
      ],
      "answer": 1
    },
    {
      "question": "Which statement about a rectangle is true?",
      "options": [
        "All four sides must be equal",
        "All four angles are 90°",
        "It has no diagonals",
        "Adjacent sides are always equal"
      ],
      "answer": 1
    },
    {
      "question": "A radius is 4 cm. What is the diameter?",
      "options": [
        "2 cm",
        "4 cm",
        "8 cm",
        "16 cm"
      ],
      "answer": 2
    },
    {
      "question": "A diameter is 14 cm. What is the radius?",
      "options": [
        "7 cm",
        "14 cm",
        "28 cm",
        "3.5 cm"
      ],
      "answer": 0
    },
    {
      "question": "Which pair of lines is perpendicular?",
      "options": [
        "Lines meeting at 30°",
        "Lines meeting at 45°",
        "Lines meeting at 90°",
        "Parallel lines"
      ],
      "answer": 2
    },
    {
      "question": "Which shape is a special type of rectangle?",
      "options": [
        "Circle",
        "Square",
        "Triangle",
        "Pentagon"
      ],
      "answer": 1
    },
    {
      "question": "What is a diagonal?",
      "options": [
        "A side of a circle",
        "A segment joining non-adjacent vertices",
        "A radius only",
        "A curved boundary"
      ],
      "answer": 1
    }
  ],
  "hard": [
    {
      "question": "A circle has diameter 18 cm. Its radius is:",
      "options": [
        "6 cm",
        "9 cm",
        "18 cm",
        "36 cm"
      ],
      "answer": 1
    },
    {
      "question": "A square has side 6 cm. Its perimeter is:",
      "options": [
        "12 cm",
        "18 cm",
        "24 cm",
        "36 cm"
      ],
      "answer": 2
    },
    {
      "question": "A rectangle is 8 cm long and 5 cm wide. Which pair gives its opposite side lengths?",
      "options": [
        "8 cm and 5 cm",
        "8 cm and 8 cm",
        "5 cm and 5 cm only",
        "All sides 13 cm"
      ],
      "answer": 0
    },
    {
      "question": "Which construction idea helps make two sides equal?",
      "options": [
        "Changing the compass randomly",
        "Transferring the same compass opening",
        "Guessing by eye",
        "Using a calculator"
      ],
      "answer": 1
    },
    {
      "question": "If a compass is set to 3 cm and its needle is placed at P, points traced by a full turn are:",
      "options": [
        "3 cm from P",
        "6 cm from P",
        "9 cm from P",
        "Any distance from P"
      ],
      "answer": 0
    },
    {
      "question": "Which property distinguishes a square from a general rectangle?",
      "options": [
        "It has four right angles",
        "It has opposite sides equal",
        "All four sides are equal",
        "It has diagonals"
      ],
      "answer": 2
    },
    {
      "question": "In rectangle ABCD, the diagonals are:",
      "options": [
        "AB and CD",
        "AC and BD",
        "AD and BC",
        "AB and AC"
      ],
      "answer": 1
    },
    {
      "question": "A square has side 9 cm. Each diagonal:",
      "options": [
        "Must be 9 cm",
        "Joins opposite vertices",
        "Is a side",
        "Does not exist"
      ],
      "answer": 1
    },
    {
      "question": "Which construction should be used to create the corner of a rectangle?",
      "options": [
        "A perpendicular line",
        "A random arc",
        "A circle only",
        "A parallel line only"
      ],
      "answer": 0
    },
    {
      "question": "Why should the compass needle stay fixed while drawing a circle?",
      "options": [
        "To keep the centre fixed",
        "To change the radius",
        "To shorten the circle",
        "To make a triangle"
      ],
      "answer": 0
    },
    {
      "question": "A rectangle has length 10 cm and breadth 4 cm. Its perimeter is:",
      "options": [
        "14 cm",
        "20 cm",
        "28 cm",
        "40 cm"
      ],
      "answer": 2
    },
    {
      "question": "Which statement about rectangle diagonals is correct?",
      "options": [
        "They are equal",
        "They are always perpendicular",
        "There are four",
        "They are sides"
      ],
      "answer": 0
    },
    {
      "question": "Which statement about square diagonals is correct?",
      "options": [
        "They are unequal",
        "They are equal and perpendicular",
        "There is one",
        "They are curved"
      ],
      "answer": 1
    },
    {
      "question": "To construct a square from one side, after drawing perpendiculars at both endpoints you need to:",
      "options": [
        "Mark equal side lengths on them",
        "Erase the first side",
        "Draw a circle only",
        "Measure random lengths"
      ],
      "answer": 0
    },
    {
      "question": "A complex geometric design is easiest to construct by:",
      "options": [
        "Drawing everything freehand",
        "Breaking it into simpler parts",
        "Avoiding measurements",
        "Changing the scale randomly"
      ],
      "answer": 1
    }
  ],
  "tooHard": [
    {
      "question": "A circle has radius 7.5 cm. Its diameter is:",
      "options": [
        "3.75 cm",
        "7.5 cm",
        "15 cm",
        "22.5 cm"
      ],
      "answer": 2
    },
    {
      "question": "A rectangle measures 12 cm by 7 cm. Its perimeter is:",
      "options": [
        "19 cm",
        "24 cm",
        "38 cm",
        "84 cm"
      ],
      "answer": 2
    },
    {
      "question": "A square has perimeter 36 cm. Its side is:",
      "options": [
        "6 cm",
        "9 cm",
        "12 cm",
        "18 cm"
      ],
      "answer": 1
    },
    {
      "question": "A rectangle has perimeter 30 cm and length 9 cm. Its breadth is:",
      "options": [
        "5 cm",
        "6 cm",
        "12 cm",
        "21 cm"
      ],
      "answer": 1
    },
    {
      "question": "A compass is opened to 4.2 cm. A circle is drawn. The distance from the centre to any point on the circle is:",
      "options": [
        "2.1 cm",
        "4.2 cm",
        "8.4 cm",
        "16.8 cm"
      ],
      "answer": 1
    },
    {
      "question": "Which sequence best describes constructing a square from side AB?",
      "options": [
        "Draw AB, perpendiculars at A and B, mark equal lengths, join remaining points",
        "Draw a circle, erase AB, guess corners",
        "Draw two parallel lines only",
        "Measure diagonals first and ignore sides"
      ],
      "answer": 0
    },
    {
      "question": "A rectangle has diagonal AC. The other diagonal is:",
      "options": [
        "AB",
        "BC",
        "BD",
        "CD"
      ],
      "answer": 2
    },
    {
      "question": "If a rectangle is also a square, what must be true?",
      "options": [
        "Its adjacent sides are equal",
        "Its diagonals disappear",
        "Its angles are not right angles",
        "It has five sides"
      ],
      "answer": 0
    },
    {
      "question": "A 5 cm radius circle and a 10 cm diameter circle have:",
      "options": [
        "Different centres only",
        "The same size",
        "Equal radii but different diameters",
        "No relation"
      ],
      "answer": 1
    },
    {
      "question": "Why is a ruler alone not enough for many compass constructions?",
      "options": [
        "It cannot draw circles or transfer fixed distances in the same way",
        "It cannot draw any line",
        "It cannot measure length",
        "It cannot be held"
      ],
      "answer": 0
    },
    {
      "question": "A square diagonal divides the square into:",
      "options": [
        "Two triangles",
        "Three circles",
        "Four rectangles only",
        "No regions"
      ],
      "answer": 0
    },
    {
      "question": "If a construction requires equal lengths, the most reliable method is:",
      "options": [
        "Estimate visually",
        "Transfer the length with a fixed compass opening",
        "Use different openings",
        "Draw freehand"
      ],
      "answer": 1
    },
    {
      "question": "A rectangle has sides 6 cm and 11 cm. Its area is:",
      "options": [
        "17 cm²",
        "34 cm²",
        "66 cm²",
        "132 cm²"
      ],
      "answer": 2
    },
    {
      "question": "A square has side 8 cm. Its diagonal connects:",
      "options": [
        "Adjacent vertices",
        "Opposite vertices",
        "Centre to a side",
        "Two points on one side"
      ],
      "answer": 1
    },
    {
      "question": "If two lines meet at 90°, they are:",
      "options": [
        "Parallel",
        "Perpendicular",
        "Curved",
        "Coincident"
      ],
      "answer": 1
    },
    {
      "question": "A circle is constructed with centre P and radius 6 cm. Point Q is on the circle. PQ equals:",
      "options": [
        "3 cm",
        "6 cm",
        "12 cm",
        "36 cm"
      ],
      "answer": 1
    },
    {
      "question": "Which property can be used to check a constructed rectangle?",
      "options": [
        "Opposite sides equal and four right angles",
        "All four sides equal only",
        "Three right angles only",
        "One diagonal only"
      ],
      "answer": 0
    },
    {
      "question": "Which property can be used to check a constructed square?",
      "options": [
        "All four sides equal and four right angles",
        "Only one side equal",
        "Only one angle equal",
        "No measurements"
      ],
      "answer": 0
    },
    {
      "question": "If a compass opening changes while copying a length, the copied segment may be:",
      "options": [
        "Exactly equal always",
        "Incorrect in length",
        "Automatically perpendicular",
        "A circle"
      ],
      "answer": 1
    },
    {
      "question": "A design uses six equal circles. The best way to maintain equal radii is to:",
      "options": [
        "Reset the compass randomly each time",
        "Keep the same compass opening when drawing them",
        "Use a ruler without marking",
        "Change the centre only"
      ],
      "answer": 1
    }
  ],
  "extreme": [
    {
      "question": "A rectangle has length 13 cm and breadth 7 cm. What is the perimeter?",
      "options": [
        "20 cm",
        "26 cm",
        "40 cm",
        "91 cm"
      ],
      "answer": 2
    },
    {
      "question": "A square has perimeter 52 cm. Its side length is:",
      "options": [
        "12 cm",
        "13 cm",
        "14 cm",
        "26 cm"
      ],
      "answer": 1
    },
    {
      "question": "A circle has diameter 25 cm. Its radius is:",
      "options": [
        "10 cm",
        "12.5 cm",
        "25 cm",
        "50 cm"
      ],
      "answer": 1
    },
    {
      "question": "A rectangle has perimeter 46 cm and length 15 cm. Its breadth is:",
      "options": [
        "6 cm",
        "8 cm",
        "16 cm",
        "23 cm"
      ],
      "answer": 1
    },
    {
      "question": "A square has area 81 cm². Its side is:",
      "options": [
        "8 cm",
        "9 cm",
        "18 cm",
        "40.5 cm"
      ],
      "answer": 1
    },
    {
      "question": "Which statement is always true for both a square and a rectangle?",
      "options": [
        "All four sides are equal",
        "All four angles are right angles",
        "Diagonals are perpendicular",
        "They have no diagonals"
      ],
      "answer": 1
    },
    {
      "question": "Which statement is always true for a square but not every rectangle?",
      "options": [
        "Opposite sides are equal",
        "Four right angles",
        "All four sides are equal",
        "Two diagonals"
      ],
      "answer": 2
    },
    {
      "question": "A rectangle is 12 cm × 5 cm. If both dimensions are doubled, its new perimeter is:",
      "options": [
        "34 cm",
        "68 cm",
        "120 cm",
        "240 cm"
      ],
      "answer": 1
    },
    {
      "question": "A square side is increased from 5 cm to 10 cm. Its perimeter becomes:",
      "options": [
        "2 times",
        "3 times",
        "4 times",
        "5 times"
      ],
      "answer": 0
    },
    {
      "question": "A circle has radius 3 cm. If the radius is doubled, the diameter becomes:",
      "options": [
        "3 cm",
        "6 cm",
        "9 cm",
        "12 cm"
      ],
      "answer": 3
    },
    {
      "question": "A point P is fixed. All points 8 cm from P form:",
      "options": [
        "A line segment",
        "A circle centred at P",
        "A square with centre P",
        "A triangle"
      ],
      "answer": 1
    },
    {
      "question": "A compass copies AB exactly. If AB = 7.8 cm, the copied segment should be:",
      "options": [
        "7.8 cm",
        "15.6 cm",
        "3.9 cm",
        "Any length"
      ],
      "answer": 0
    },
    {
      "question": "In a rectangle, diagonals AC and BD intersect at O. Which statement is correct?",
      "options": [
        "AC = BD and they bisect each other",
        "AC is shorter than BD",
        "They never intersect",
        "Only one diagonal exists"
      ],
      "answer": 0
    },
    {
      "question": "In a square, diagonals AC and BD intersect at O. Which combination is true?",
      "options": [
        "AC = BD; diagonals bisect each other at 90°",
        "AC is longer; no bisection",
        "They are parallel",
        "Only one diagonal"
      ],
      "answer": 0
    },
    {
      "question": "A rectangle has diagonal length 10 cm. Which fact can be used in construction?",
      "options": [
        "Both diagonals can be made equal",
        "Its sides must all be 10 cm",
        "It has no centre",
        "Its angles must be 45°"
      ],
      "answer": 0
    },
    {
      "question": "A square is constructed by making AD and BC equal to AB on perpendiculars at A and B. What should be done next?",
      "options": [
        "Join C and D",
        "Erase A and B",
        "Draw a fifth side",
        "Change AD"
      ],
      "answer": 0
    },
    {
      "question": "A rectangle is constructed with AB = 9 cm and AD = 4 cm. Which is true?",
      "options": [
        "BC = 4 cm and CD = 9 cm",
        "BC = 9 cm and CD = 4 cm",
        "All sides 4 cm",
        "All sides 9 cm"
      ],
      "answer": 0
    },
    {
      "question": "A geometric pattern uses a repeated compass opening of 2 cm. If one opening is changed to 3 cm, what is the likely effect?",
      "options": [
        "That part will not match the intended equal-distance pattern",
        "Nothing ever changes",
        "It becomes a right angle automatically",
        "All lines disappear"
      ],
      "answer": 0
    },
    {
      "question": "A student draws a circle but moves the compass needle during the turn. The result may not be:",
      "options": [
        "A perfect circle with one fixed centre",
        "A line",
        "A square",
        "A rectangle"
      ],
      "answer": 0
    },
    {
      "question": "Which is the strongest reason constructions are useful?",
      "options": [
        "They make geometric relationships precise and reproducible",
        "They remove the need to understand shapes",
        "They always require computers",
        "They make measurements unnecessary"
      ],
      "answer": 0
    },
    {
      "question": "A rectangle has length 18 cm and breadth 7 cm. If a square is made with side equal to the breadth, what is the square perimeter?",
      "options": [
        "14 cm",
        "28 cm",
        "36 cm",
        "49 cm"
      ],
      "answer": 1
    },
    {
      "question": "A square of side 12 cm and a rectangle 14 cm × 10 cm have perimeters:",
      "options": [
        "Both 48 cm",
        "Square 48 cm and rectangle 48 cm",
        "Square 24 cm and rectangle 48 cm",
        "Square 48 cm and rectangle 24 cm"
      ],
      "answer": 1
    },
    {
      "question": "A circle of radius 5 cm is drawn. A second circle has the same centre and radius 10 cm. Compared with the first, the second circle has:",
      "options": [
        "The same diameter",
        "Double the radius and double the diameter",
        "Half the radius",
        "No fixed distance"
      ],
      "answer": 1
    },
    {
      "question": "If a construction needs a point equidistant from two given points, which tool is especially useful?",
      "options": [
        "Compass",
        "Eraser",
        "Calculator",
        "Notebook"
      ],
      "answer": 0
    },
    {
      "question": "A figure is constructed from a rectangle, a triangle and circles. The best checking strategy is to:",
      "options": [
        "Check each basic part and then the complete figure",
        "Only look at the final picture",
        "Ignore measurements",
        "Check one random point"
      ],
      "answer": 0
    }
  ]
}
;

/* ==================================================
   CLASS 6 • MATHEMATICS • CHAPTER 9
   Symmetry — Package 13.1.9
================================================== */
quizDatabase.class6.mathematics["symmetry"] = {
  easy: [
    {question:"What is symmetry?",options:["A random drawing","A regular correspondence between parts of a figure","A type of measurement","Only a curved line"],answer:1},
    {question:"A line that divides a figure into mirror-matching parts is called a:",options:["Number line","Line of symmetry","Diagonal only","Radius"],answer:1},
    {question:"How many lines of symmetry does a square have?",options:["1","2","3","4"],answer:3},
    {question:"How many lines of symmetry does a non-square rectangle have?",options:["1","2","3","4"],answer:1},
    {question:"How many lines of symmetry does an equilateral triangle have?",options:["1","2","3","4"],answer:2},
    {question:"Every diameter of a circle is a:",options:["Line of symmetry","Side","Vertex","Diagonal of a square"],answer:0},
    {question:"Reflection symmetry is related to:",options:["Mirroring across a line","Only adding numbers","Measuring area","Counting vertices"],answer:0},
    {question:"Rotational symmetry involves:",options:["Folding only","Turning a figure about a centre","Changing its size","Erasing a side"],answer:1},
    {question:"What is the centre about which a figure is turned called?",options:["Centre of rotation","Endpoint","Vertex only","Midpoint of every side"],answer:0},
    {question:"A square has rotational symmetry of order:",options:["2","3","4","5"],answer:2}
  ],
  medium: [
    {question:"Which test is useful for checking line symmetry?",options:["Fold along the proposed line and see if halves coincide","Measure only one side","Count colours","Rotate only 360°"],answer:0},
    {question:"A point and its reflection across a symmetry line are:",options:["At unequal distances from the line","Equally distant from the line","Always the same point","On the same side"],answer:1},
    {question:"Which shape has exactly two lines of symmetry?",options:["A non-square rectangle","A square","An equilateral triangle","A circle"],answer:0},
    {question:"The smallest non-zero angle of rotation of a square is:",options:["45°","90°","120°","180°"],answer:1},
    {question:"The smallest non-zero angle of rotation of an equilateral triangle is:",options:["60°","90°","120°","180°"],answer:2},
    {question:"A shape matches itself after a 180° turn. Which conclusion is valid?",options:["It has rotational symmetry of at least order 2","It must have four lines of symmetry","It cannot have reflection symmetry","It is a circle"],answer:0},
    {question:"Which statement about a circle is correct?",options:["It has exactly 2 lines of symmetry","It has no rotational symmetry","It has infinitely many lines of symmetry","It has exactly 4 lines of symmetry"],answer:2},
    {question:"If a figure has 4 matching positions in one full turn, its rotational order is:",options:["2","3","4","8"],answer:2},
    {question:"Which is NOT enough to prove a line is a line of symmetry?",options:["The figure looks balanced","The two halves coincide when folded","Corresponding points are equally distant from the line","The line is actually a mirror axis"],answer:0},
    {question:"A square's diagonals are:",options:["Two lines of symmetry","Never lines of symmetry","Parallel sides","Radii"],answer:0}
  ],
  hard: [
    {question:"A regular polygon has rotational symmetry of order 5. Its smallest non-zero angle of rotation is:",options:["36°","60°","72°","90°"],answer:2},
    {question:"A regular polygon has rotational symmetry of order 6. Its smallest non-zero angle is:",options:["30°","45°","60°","120°"],answer:2},
    {question:"A figure has exactly one line of symmetry and rotational order 1. Which is possible?",options:["A typical isosceles triangle","A square","A circle","A regular hexagon"],answer:0},
    {question:"Why can a non-square rectangle not use its diagonal as a line of symmetry?",options:["The reflected halves do not coincide","Its diagonal is always horizontal","A rectangle has no diagonals","Diagonals cannot be drawn"],answer:0},
    {question:"If a point is 3 cm from a line of symmetry, its reflected image is:",options:["3 cm from the line on the opposite side","6 cm from the line on the same side","1.5 cm away","At an arbitrary distance"],answer:0},
    {question:"A square is rotated through 270°. What happens?",options:["It does not match","It matches its original appearance","It becomes a rectangle","It becomes a triangle"],answer:1},
    {question:"A shape matches itself at 120°, 240° and 360°. What is its rotational order?",options:["2","3","4","6"],answer:1},
    {question:"A shape matches itself at 60° intervals. What is its rotational order?",options:["3","4","5","6"],answer:3},
    {question:"Which pair has the same number of lines of symmetry?",options:["Square and equilateral triangle","Rectangle and square","Equilateral triangle and its rotated copy","Circle and square"],answer:2},
    {question:"A design is made by rotating one motif equally around a centre. The strongest symmetry to investigate first is:",options:["Rotational symmetry","Only reflection symmetry","Only translation","No symmetry"],answer:0},
    {question:"If a figure has rotational order 4, the angle between consecutive matching positions is:",options:["45°","60°","90°","120°"],answer:2},
    {question:"Which statement best distinguishes reflection from rotation?",options:["Reflection uses a line as an axis; rotation uses a centre","Both always use a line","Rotation changes size; reflection changes size","Reflection requires 360°"],answer:0},
    {question:"A circle can be rotated by 30° about its centre and still match itself because:",options:["Every point remains at the same distance from the centre","It has four sides","It has one line of symmetry","Its radius becomes zero"],answer:0},
    {question:"A student says a figure is symmetric because its left and right sides look similar. Best response?",options:["Verify the exact reflection relationship","Always accept the claim","Count colours only","Measure area only"],answer:0},
    {question:"Which construction on a grid most directly creates reflection symmetry?",options:["Copy each point at the same perpendicular distance across an axis","Move every point randomly","Stretch one side","Rotate every point by a different angle"],answer:0}
  ],
  tooHard: [
    {question:"A figure has rotational order 8. How many degrees separate consecutive matching positions?",options:["30°","45°","60°","90°"],answer:1},
    {question:"A regular polygon has 10-fold rotational symmetry. Its smallest non-zero rotation is:",options:["18°","30°","36°","72°"],answer:2},
    {question:"A figure has rotational order 12. Which angle is NOT a smallest matching angle but is still a matching rotation?",options:["30°","60°","90°","360°"],answer:3},
    {question:"A shape has 5 lines of reflection symmetry and rotational order 5. Which type of regular figure is consistent with this pattern?",options:["Regular pentagon","Non-square rectangle","Equilateral triangle","Square"],answer:0},
    {question:"A regular hexagon has how many lines of symmetry?",options:["3","4","5","6"],answer:3},
    {question:"A regular hexagon has rotational order:",options:["3","4","5","6"],answer:3},
    {question:"A regular octagon has how many lines of symmetry?",options:["4","6","8","10"],answer:2},
    {question:"A regular octagon has smallest non-zero angle of rotation:",options:["30°","45°","60°","90°"],answer:1},
    {question:"If a figure is reflected twice in the same line, the final figure is:",options:["The original figure","A 90° rotation","A larger figure","Always a different figure"],answer:0},
    {question:"A point lies exactly on the line of reflection. Where does its reflected image lie?",options:["At the same point","2 cm away always","On the opposite side","At the centre of the figure"],answer:0},
    {question:"A square is rotated 180° and then 90°. The total turn is equivalent to:",options:["90°","180°","270°","360°"],answer:2},
    {question:"A figure has rotational order 3. Which angle must produce a match?",options:["60°","90°","120°","150°"],answer:2},
    {question:"A figure has rotational order 4. Which set contains all non-zero matching angles below 360°?",options:["90°, 180°, 270°","45°, 90°, 135°","60°, 120°, 180°","120°, 240°"],answer:0},
    {question:"A square has four reflection axes and rotational order four. Which conclusion is correct?",options:["Its reflection and rotational symmetries are both non-trivial","It has no symmetry","It has only one type of symmetry","Its sides need not be equal"],answer:0},
    {question:"A design has a centre and repeats after every 72°. What is its rotational order?",options:["4","5","6","8"],answer:1},
    {question:"If a figure has rotational order 6, how many non-zero matching rotations are there before 360°?",options:["5","6","7","12"],answer:0},
    {question:"A regular triangle has 3 reflection axes. Its rotational order is also 3 because:",options:["Its three vertices can occupy matching positions after equal 120° turns","It has four sides","All triangles have order 3","Reflection and rotation are identical"],answer:0},
    {question:"Which operation preserves every distance between points?",options:["Reflection and rotation","Random stretching","Unequal scaling","Shearing"],answer:0},
    {question:"A reflected point is 4 cm from the axis. The distance between the point and its image measured straight across the axis is:",options:["2 cm","4 cm","8 cm","16 cm"],answer:2},
    {question:"A square has side 6 cm. Which fact is relevant when testing its diagonal as a symmetry line?",options:["The two triangular halves are congruent mirror images","Its diagonal must be a side","A diagonal is a radius","A square has no diagonals"],answer:0}
  ],
  extreme: [
    {question:"A regular n-sided polygon has rotational order n. If its smallest angle of rotation is 24°, n equals:",options:["12","15","18","24"],answer:1},
    {question:"A regular polygon has 15-fold rotational symmetry. Its smallest angle is:",options:["12°","18°","24°","30°"],answer:0},
    {question:"A point P is 7 cm from a reflection axis. Point P' is its image. What is PP' if the segment joining them is perpendicular to the axis?",options:["7 cm","10 cm","14 cm","21 cm"],answer:2},
    {question:"A figure has rotational order 8. Which angle is the third smallest positive matching rotation?",options:["45°","90°","135°","180°"],answer:2},
    {question:"A figure has rotational order 9. Which angle is the fourth smallest positive matching rotation?",options:["80°","120°","160°","200°"],answer:1},
    {question:"A regular decagon has 10 reflection axes. Its rotational order is:",options:["5","8","10","20"],answer:2},
    {question:"If a figure has both a line of symmetry and rotational order 2, which statement is necessarily true?",options:["It matches after a 180° turn and at least one reflection","It must be a circle","It must have four sides","It must have infinitely many axes"],answer:0},
    {question:"A pattern repeats around a point five times equally. What is the central angle between adjacent copies?",options:["36°","60°","72°","90°"],answer:2},
    {question:"A square is reflected in one diagonal and then reflected in the other diagonal. The combined effect is equivalent to:",options:["A 90° rotation","A 180° rotation","A 270° rotation","No movement"],answer:1},
    {question:"A rectangle is reflected in its vertical symmetry line and then its horizontal symmetry line. The combined effect is:",options:["A 90° rotation","A 180° rotation","A 45° rotation","A scaling"],answer:1},
    {question:"A regular hexagon is rotated through 240°. How many of the six vertex positions move onto original vertex positions?",options:["2","3","6","None"],answer:2},
    {question:"A regular pentagon is rotated through 144°. How many times does the figure have matched itself at or before this turn, excluding 0°?",options:["1","2","3","4"],answer:1},
    {question:"A figure has 12 matching positions in 360°. Which angle is the seventh positive matching rotation?",options:["150°","180°","210°","240°"],answer:2},
    {question:"A shape has 4 reflection axes equally spaced around its centre. What angle separates adjacent axes?",options:["45°","60°","90°","120°"],answer:2},
    {question:"A regular polygon has 18 sides. Its smallest rotational angle is:",options:["10°","18°","20°","30°"],answer:2},
    {question:"A design has order 6 rotational symmetry and is reflected across a line through its centre. Which is a reasonable conclusion?",options:["The reflection may produce another symmetry of the design, but it must be checked","Reflection always destroys rotational symmetry","The design becomes a circle","Order must become 1"],answer:0},
    {question:"A figure has exactly 2 lines of symmetry and rotational order 2. Which common shape fits?",options:["A non-square rectangle","A square","A circle","An equilateral triangle"],answer:0},
    {question:"A figure has exactly 3 lines of symmetry and rotational order 3. Which common shape fits?",options:["Equilateral triangle","Non-square rectangle","Square","Circle"],answer:0},
    {question:"A figure has exactly 4 lines of symmetry and rotational order 4. Which common shape fits?",options:["Square","Rectangle that is not a square","Equilateral triangle","Scalene triangle"],answer:0},
    {question:"Which statement is the strongest general rule for a rigid reflection or rotation?",options:["Lengths and angles are preserved, while orientation may change under reflection","Lengths always change","Angles always become zero","Area always becomes double"],answer:0},
    {question:"A student rotates a square by 450°. What is the equivalent turn within one 360° cycle?",options:["45°","90°","180°","270°"],answer:1},
    {question:"A student rotates a regular pentagon by 540°. What is the equivalent turn within one 360° cycle?",options:["36°","72°","180°","360°"],answer:2},
    {question:"A figure has smallest positive rotational angle 40°. What is its rotational order?",options:["6","8","9","10"],answer:3},
    {question:"A figure has smallest positive rotational angle 30°. What is its rotational order?",options:["10","12","15","30"],answer:1},
    {question:"A point is reflected across x=0 from coordinate (5, -2). Its image is:",options:["(5,2)","(-5,-2)","(-5,2)","(2,-5)"],answer:1}
  ]
};


quizDatabase.class6.mathematics["class6-mathematics-the-other-side-of-zero"] = {
  "easy": [
    {
      "question": "What is an integer?",
      "options": [
        "A whole number that may be negative, zero or positive",
        "Only a positive number",
        "Only a fraction",
        "Only a decimal"
      ],
      "answer": 0
    },
    {
      "question": "Which number is negative?",
      "options": [
        "5",
        "0",
        "-3",
        "8"
      ],
      "answer": 2
    },
    {
      "question": "Where is -4 on a number line?",
      "options": [
        "Right of 0",
        "Left of 0",
        "At 0",
        "It cannot be placed"
      ],
      "answer": 1
    },
    {
      "question": "What is the opposite of 7?",
      "options": [
        "7",
        "-7",
        "0",
        "1"
      ],
      "answer": 1
    },
    {
      "question": "What is the opposite of -9?",
      "options": [
        "-9",
        "0",
        "9",
        "1"
      ],
      "answer": 2
    },
    {
      "question": "Which is greater?",
      "options": [
        "-8",
        "-2",
        "They are equal",
        "Cannot compare"
      ],
      "answer": 1
    },
    {
      "question": "What is 0 + (-5)?",
      "options": [
        "5",
        "-5",
        "0",
        "-10"
      ],
      "answer": 1
    },
    {
      "question": "What is 4 + 3?",
      "options": [
        "6",
        "7",
        "8",
        "1"
      ],
      "answer": 1
    },
    {
      "question": "What is -2 + (-3)?",
      "options": [
        "-5",
        "5",
        "1",
        "-1"
      ],
      "answer": 0
    },
    {
      "question": "What is 6 - 2?",
      "options": [
        "2",
        "3",
        "4",
        "8"
      ],
      "answer": 2
    }
  ],
  "medium": [
    {
      "question": "What is 7 + (-10)?",
      "options": [
        "17",
        "-17",
        "3",
        "-3"
      ],
      "answer": 3
    },
    {
      "question": "What is -8 + 5?",
      "options": [
        "-13",
        "13",
        "-3",
        "3"
      ],
      "answer": 2
    },
    {
      "question": "What is -6 + (-7)?",
      "options": [
        "13",
        "-13",
        "-1",
        "1"
      ],
      "answer": 1
    },
    {
      "question": "What is 9 - (-4)?",
      "options": [
        "5",
        "13",
        "-13",
        "-5"
      ],
      "answer": 1
    },
    {
      "question": "What is -5 - 6?",
      "options": [
        "1",
        "-1",
        "11",
        "-11"
      ],
      "answer": 3
    },
    {
      "question": "Which integer is greatest?",
      "options": [
        "-12",
        "-5",
        "-9",
        "-20"
      ],
      "answer": 1
    },
    {
      "question": "Which integer is smallest?",
      "options": [
        "-1",
        "-7",
        "3",
        "-4"
      ],
      "answer": 1
    },
    {
      "question": "What is the absolute value of -14?",
      "options": [
        "-14",
        "14",
        "0",
        "1"
      ],
      "answer": 1
    },
    {
      "question": "If 0 is ground floor, what does -2 represent?",
      "options": [
        "Two floors above",
        "Two floors below",
        "Two metres above",
        "No position"
      ],
      "answer": 1
    },
    {
      "question": "Which expression equals zero?",
      "options": [
        "6+6",
        "6+(-6)",
        "-6-6",
        "0-6"
      ],
      "answer": 1
    }
  ],
  "hard": [
    {
      "question": "A temperature changes from -3°C to 5°C. What is the increase?",
      "options": [
        "2°C",
        "8°C",
        "-8°C",
        "15°C"
      ],
      "answer": 1
    },
    {
      "question": "What is -12 + 7 - 3?",
      "options": [
        "-8",
        "-2",
        "-22",
        "2"
      ],
      "answer": 0
    },
    {
      "question": "What is 15 - (-7) + (-4)?",
      "options": [
        "4",
        "18",
        "26",
        "-18"
      ],
      "answer": 1
    },
    {
      "question": "Arrange from least to greatest: -2, 5, -7, 0.",
      "options": [
        "5, 0, -2, -7",
        "-7, -2, 0, 5",
        "-2, -7, 0, 5",
        "0, -2, 5, -7"
      ],
      "answer": 1
    },
    {
      "question": "Which statement is true?",
      "options": [
        "-10 > -2",
        "-10 < -2",
        "-10 = -2",
        "-10 > 0"
      ],
      "answer": 1
    },
    {
      "question": "A diver is at -18 m and rises 7 m. Where is the diver?",
      "options": [
        "-25 m",
        "-11 m",
        "11 m",
        "25 m"
      ],
      "answer": 1
    },
    {
      "question": "A bank balance changes by -₹350 and then +₹500. Net change is:",
      "options": [
        "-₹850",
        "₹150",
        "₹850",
        "-₹150"
      ],
      "answer": 1
    },
    {
      "question": "What is -4 - (-9)?",
      "options": [
        "-13",
        "13",
        "5",
        "-5"
      ],
      "answer": 2
    },
    {
      "question": "If x + (-8) = -3, x is:",
      "options": [
        "-11",
        "-5",
        "5",
        "11"
      ],
      "answer": 2
    },
    {
      "question": "Which pair are additive inverses?",
      "options": [
        "7 and 7",
        "-4 and -4",
        "9 and -9",
        "0 and 1"
      ],
      "answer": 2
    },
    {
      "question": "A lift moves from floor -3 to floor 4. How many floors does it move upward?",
      "options": [
        "1",
        "4",
        "7",
        "-7"
      ],
      "answer": 2
    },
    {
      "question": "What is (-20) + 8 + 5?",
      "options": [
        "-27",
        "-7",
        "7",
        "-13"
      ],
      "answer": 1
    },
    {
      "question": "Which number is 6 units left of -2?",
      "options": [
        "4",
        "-8",
        "-4",
        "8"
      ],
      "answer": 1
    },
    {
      "question": "Which number is 9 units right of -5?",
      "options": [
        "-14",
        "4",
        "14",
        "-4"
      ],
      "answer": 1
    },
    {
      "question": "If a = -6 and b = -2, what is a - b?",
      "options": [
        "-8",
        "8",
        "-4",
        "4"
      ],
      "answer": 3
    }
  ],
  "tooHard": [
    {
      "question": "A number is 8 units from zero and is negative. Which is it?",
      "options": [
        "8",
        "-8",
        "0",
        "±8"
      ],
      "answer": 1
    },
    {
      "question": "If x is 5 units left of -3, x equals:",
      "options": [
        "2",
        "-2",
        "-8",
        "8"
      ],
      "answer": 2
    },
    {
      "question": "A temperature rises from -12°C by 19°C. Final temperature?",
      "options": [
        "-31°C",
        "-7°C",
        "7°C",
        "31°C"
      ],
      "answer": 2
    },
    {
      "question": "A person has ₹200, spends ₹350, then receives ₹100. Net balance change from the starting amount is:",
      "options": [
        "-₹50",
        "₹50",
        "-₹450",
        "₹450"
      ],
      "answer": 0
    },
    {
      "question": "Which is the correct equivalent form of -9 - (-14)?",
      "options": [
        "-9 + (-14)",
        "-9 + 14",
        "9 + (-14)",
        "9 + 14"
      ],
      "answer": 1
    },
    {
      "question": "If a - (-7) = -2, a equals:",
      "options": [
        "-9",
        "-5",
        "5",
        "9"
      ],
      "answer": 0
    },
    {
      "question": "Three consecutive integers have sum -12. What are they?",
      "options": [
        "-3,-4,-5",
        "-2,-4,-6",
        "-5,-6,-1",
        "0,-4,-8"
      ],
      "answer": 0
    },
    {
      "question": "The distance between -7 and 5 on a number line is:",
      "options": [
        "2",
        "12",
        "-12",
        "35"
      ],
      "answer": 1
    },
    {
      "question": "Which expression has value -1?",
      "options": [
        "-5+4",
        "5-4",
        "-5-4",
        "4+5"
      ],
      "answer": 0
    },
    {
      "question": "If -a is the opposite of a, what is the opposite of -(-6)?",
      "options": [
        "-6",
        "6",
        "0",
        "12"
      ],
      "answer": 0
    },
    {
      "question": "A lift starts at -4, rises 9, then descends 6. Final floor?",
      "options": [
        "-1",
        "-7",
        "11",
        "7"
      ],
      "answer": 0
    },
    {
      "question": "Which statement about absolute value is correct?",
      "options": [
        "It can be negative",
        "It is the distance from zero and is never negative",
        "It is always zero",
        "It changes sign"
      ],
      "answer": 1
    },
    {
      "question": "If p=-15 and q=9, which is greater?",
      "options": [
        "p",
        "q",
        "They are equal",
        "Cannot tell"
      ],
      "answer": 1
    },
    {
      "question": "What is the result of -18 - (-5) + 9?",
      "options": [
        "-22",
        "-4",
        "4",
        "22"
      ],
      "answer": 1
    },
    {
      "question": "A number plus its opposite is always:",
      "options": [
        "1",
        "-1",
        "0",
        "The number itself"
      ],
      "answer": 2
    },
    {
      "question": "Which sequence moves steadily right by 4 on the number line?",
      "options": [
        "-10,-6,-2,2",
        "10,6,2,-2",
        "-10,-5,0,6",
        "-2,-6,-10,-14"
      ],
      "answer": 0
    },
    {
      "question": "If the final value is -3 after adding 8 to an integer, the starting integer was:",
      "options": [
        "5",
        "-5",
        "11",
        "-11"
      ],
      "answer": 1
    },
    {
      "question": "A submarine goes from -25 m to -40 m. Its change in position is:",
      "options": [
        "+15 m",
        "-15 m",
        "+65 m",
        "-65 m"
      ],
      "answer": 1
    },
    {
      "question": "Which calculation demonstrates subtracting a negative correctly?",
      "options": [
        "6-(-4)=2",
        "6-(-4)=10",
        "6-(-4)=-10",
        "6-(-4)=-2"
      ],
      "answer": 1
    },
    {
      "question": "If |x|=7, the possible integer values of x are:",
      "options": [
        "7 only",
        "-7 only",
        "7 and -7",
        "0 and 7"
      ],
      "answer": 2
    }
  ],
  "extreme": [
    {
      "question": "Find x if x - (-13) = -4.",
      "options": [
        "-17",
        "-9",
        "9",
        "17"
      ],
      "answer": 0
    },
    {
      "question": "Find x if -8 - x = 5.",
      "options": [
        "13",
        "-13",
        "3",
        "-3"
      ],
      "answer": 1
    },
    {
      "question": "Three consecutive integers sum to -45. The middle integer is:",
      "options": [
        "-16",
        "-15",
        "-14",
        "-13"
      ],
      "answer": 1
    },
    {
      "question": "Four consecutive integers have sum -26. The integers are:",
      "options": [
        "-8,-7,-6,-5",
        "-5,-6,-7,-8",
        "-10,-7,-6,-3",
        "-9,-8,-7,-2"
      ],
      "answer": 0
    },
    {
      "question": "A temperature is -18°C. It rises 11°C, falls 7°C and rises 9°C. Final temperature?",
      "options": [
        "-23°C",
        "-5°C",
        "-15°C",
        "5°C"
      ],
      "answer": 1
    },
    {
      "question": "Starting at -12, move 7 right, 15 left, then 20 right. Final position?",
      "options": [
        "0",
        "-20",
        "20",
        "-2"
      ],
      "answer": 0
    },
    {
      "question": "If |a|=9 and a<0, what is a+14?",
      "options": [
        "-23",
        "-5",
        "5",
        "23"
      ],
      "answer": 1
    },
    {
      "question": "If b is 6 units to the left of -10, what is b - (-4)?",
      "options": [
        "-12",
        "-16",
        "-4",
        "4"
      ],
      "answer": 0
    },
    {
      "question": "A balance is -₹750. A deposit of ₹1200 is followed by a withdrawal of ₹500. Final balance?",
      "options": [
        "-₹50",
        "₹50",
        "-₹950",
        "₹950"
      ],
      "answer": 0
    },
    {
      "question": "What integer must be added to -37 to get 12?",
      "options": [
        "-49",
        "-25",
        "25",
        "49"
      ],
      "answer": 2
    },
    {
      "question": "Which value of x makes |x-3|=5?",
      "options": [
        "x=8 only",
        "x=-2 only",
        "x=8 or x=-2",
        "x=2 or x=-8"
      ],
      "answer": 2
    },
    {
      "question": "A point at -6 is reflected to its opposite on a number line. The distance travelled is:",
      "options": [
        "6",
        "-6",
        "12",
        "0"
      ],
      "answer": 2
    },
    {
      "question": "If a+b=-3 and a=-11, b equals:",
      "options": [
        "-14",
        "8",
        "14",
        "-8"
      ],
      "answer": 1
    },
    {
      "question": "If m-n=-20 and n=-7, m equals:",
      "options": [
        "-27",
        "-13",
        "13",
        "27"
      ],
      "answer": 1
    },
    {
      "question": "A sequence starts -50 and increases by 13 each step. What is the fourth term?",
      "options": [
        "-24",
        "-11",
        "-37",
        "-63"
      ],
      "answer": 0
    },
    {
      "question": "A number is 4 units right of -9. Its opposite is:",
      "options": [
        "5",
        "-5",
        "13",
        "-13"
      ],
      "answer": 1
    },
    {
      "question": "What is (-3) - [(-8)+5]?",
      "options": [
        "0",
        "-10",
        "10",
        "-16"
      ],
      "answer": 2
    },
    {
      "question": "What is 12 + [(-7)-9]?",
      "options": [
        "-4",
        "-2",
        "14",
        "28"
      ],
      "answer": 0
    },
    {
      "question": "If x+(-x)=0 for every integer x, this illustrates:",
      "options": [
        "Commutativity only",
        "Additive inverse property",
        "Division",
        "Absolute value"
      ],
      "answer": 1
    },
    {
      "question": "A lift goes from floor 6 to -4, then to -1. Its total signed change is:",
      "options": [
        "-15",
        "-7",
        "7",
        "15"
      ],
      "answer": 1
    },
    {
      "question": "A runner moves 18 m west, 25 m east and 9 m west relative to start. Taking east as positive, final position is:",
      "options": [
        "-2 m",
        "2 m",
        "16 m",
        "34 m"
      ],
      "answer": 1
    },
    {
      "question": "If the sum of two integers is -8 and one is 13, the other is:",
      "options": [
        "21",
        "5",
        "-21",
        "-5"
      ],
      "answer": 2
    },
    {
      "question": "If a=-4, b=7, c=-9, find a+b-c.",
      "options": [
        "-20",
        "12",
        "2",
        "-2"
      ],
      "answer": 2
    },
    {
      "question": "Which expression equals 0?",
      "options": [
        "-15+15",
        "-15-15",
        "15+15",
        "0-15"
      ],
      "answer": 0
    },
    {
      "question": "If x is an integer and x is both greater than -2 and less than 3, how many possible integer values are there?",
      "options": [
        "3",
        "4",
        "5",
        "6"
      ],
      "answer": 2
    }
  ]
};


/* =========================================================
   CLASS 6 SCIENCE — CHAPTER 2
   Diversity in the Living World — 125-question quiz bank
   ========================================================= */
quizDatabase.class6 = quizDatabase.class6 || {};
quizDatabase.class6.science = quizDatabase.class6.science || {};
quizDatabase.class6.science["6-science-diversity-in-the-living-world"] = {"easy":[{"question":"What does biodiversity mean?","options":["Variety of living organisms","Only variety of plants","Only variety of animals","Number of habitats"],"answer":0},{"question":"What is a habitat?","options":["A place where an organism lives","A body part","A food type","A weather event"],"answer":0},{"question":"Which is a living organism?","options":["Rock","Mango tree","Plastic bottle","Water"],"answer":1},{"question":"Which is a non-living thing?","options":["Mushroom","Ant","Stone","Grass"],"answer":2},{"question":"Which feature is common to living organisms?","options":["They grow","They are always green","They can all fly","They all live on land"],"answer":0},{"question":"Which plant is generally a herb?","options":["Mint","Mango","Neem","Banyan"],"answer":0},{"question":"Which plant is generally a climber?","options":["Money plant","Wheat","Mango","Cactus"],"answer":0},{"question":"Which animal is a herbivore?","options":["Tiger","Cow","Eagle","Cat"],"answer":1},{"question":"Which animal is a carnivore?","options":["Deer","Cow","Lion","Goat"],"answer":2},{"question":"Which animal is an omnivore?","options":["Human","Cow","Deer","Rabbit"],"answer":0},{"question":"Which habitat is mainly aquatic?","options":["Desert","Pond","Mountain","Grassland"],"answer":1},{"question":"Which adaptation helps fish live in water?","options":["Gills","Fur","Hooves","Feathers"],"answer":0},{"question":"Which animal is well adapted to a desert?","options":["Camel","Penguin","Frog","Whale"],"answer":0},{"question":"Which animal is adapted to very cold regions?","options":["Polar bear","Camel","Lizard","Monkey"],"answer":0},{"question":"Why are organisms classified?","options":["To make study and identification easier","To make them identical","To stop diversity","To remove habitats"],"answer":0},{"question":"Which is a microorganism?","options":["Amoeba","Elephant","Mango tree","Sparrow"],"answer":0},{"question":"Which microorganism is commonly used in bread making?","options":["Yeast","Earthworm","Tiger","Grass"],"answer":0},{"question":"What is an ecosystem?","options":["Living and non-living components interacting","Only animals in an area","Only plants in a forest","A single organism"],"answer":0},{"question":"Which is a producer?","options":["Green plant","Tiger","Mushroom","Human"],"answer":0},{"question":"Which is a consumer?","options":["Grass","Algae","Deer","Tree"],"answer":2},{"question":"Which organism helps decompose dead matter?","options":["Fungi","Tiger","Eagle","Cow"],"answer":0},{"question":"What does a food chain show?","options":["Transfer of food and energy between organisms","Only animal movement","Only plant growth","Weather changes"],"answer":0},{"question":"Why is biodiversity important?","options":["It supports healthy ecosystems","It removes all competition","It stops adaptation","It makes every species identical"],"answer":0},{"question":"Which action can reduce biodiversity?","options":["Deforestation","Planting native trees","Protecting habitats","Reducing pollution"],"answer":0},{"question":"Which action helps conservation?","options":["Protecting wildlife habitats","Destroying forests","Overhunting","Dumping waste"],"answer":0}],"medium":[{"question":"A habitat provides an organism mainly with what?","options":["Food, water, shelter and suitable conditions","Only food","Only sunlight","Only protection from predators"],"answer":0},{"question":"Why can two organisms in different habitats have different adaptations?","options":["Their survival conditions differ","All organisms need different planets","Adaptations are random decorations","Habitats do not affect organisms"],"answer":0},{"question":"Which observation best shows diversity?","options":["A garden contains many kinds of plants and animals","A garden has one mango tree","A rock is beside a plant","One bird sits on a branch"],"answer":0},{"question":"Why is a cactus suited to a dry habitat?","options":["It has features that help conserve water","It needs no sunlight","It can breathe underwater","It has webbed feet"],"answer":0},{"question":"Why does a fish have gills?","options":["To obtain oxygen from water","To eat plants","To walk on land","To keep its body warm"],"answer":0},{"question":"Which group contains only herbivores?","options":["Cow, deer, goat","Tiger, lion, deer","Human, cow, tiger","Eagle, fox, lion"],"answer":0},{"question":"Which group contains only carnivores?","options":["Lion, tiger, eagle","Cow, goat, deer","Human, bear, cow","Rabbit, deer, horse"],"answer":0},{"question":"Which statement about omnivores is correct?","options":["They can eat both plant and animal foods","They eat only plants","They eat only animals","They cannot digest food"],"answer":0},{"question":"Why is classification useful to scientists?","options":["It organizes organisms using similarities and differences","It makes organisms evolve faster","It eliminates species","It changes habitats"],"answer":0},{"question":"A pond contains plants, fish, insects, water and mud. Which statement is best?","options":["It contains both living and non-living components","Everything in it is living","Everything in it is non-living","Only fish matter"],"answer":0},{"question":"What role do decomposers play?","options":["They break down dead organic matter and recycle nutrients","They hunt every animal","They make sunlight","They stop food chains"],"answer":0},{"question":"Which organism is most likely a producer?","options":["Algae","Frog","Fish","Bacteria-eating insect"],"answer":0},{"question":"What happens if a major plant species disappears from a food web?","options":["Consumers depending on it may be affected","Nothing can change","Only rocks are affected","The ecosystem becomes non-living instantly"],"answer":0},{"question":"Why can habitat destruction threaten species?","options":["It removes resources and places needed for survival","It always increases food","It creates unlimited shelter","It stops all competition"],"answer":0},{"question":"Which is a behavioural adaptation?","options":["A bird migrating to a warmer region","A fish having fins","A cactus having spines","A polar bear having thick fur"],"answer":0},{"question":"Which is a structural adaptation?","options":["Webbed feet in a duck","Migration","Hibernation","Changing feeding time"],"answer":0},{"question":"Why are microorganisms important?","options":["Some help in food production and decomposition","All cause diseases","They are all harmful","They live only in laboratories"],"answer":0},{"question":"Which is a useful microorganism?","options":["Lactobacillus in curd making","A disease-causing pathogen","A plastic particle","A stone"],"answer":0},{"question":"Why should food chains be viewed as connected?","options":["One organism may depend on another for food","Every organism eats every other organism","Plants do not interact","Predators are unrelated to prey"],"answer":0},{"question":"What is the best example of conservation?","options":["Creating and protecting a wildlife sanctuary","Clearing a forest","Overfishing","Poaching"],"answer":0},{"question":"How can pollution affect biodiversity?","options":["It can damage habitats and harm organisms","It always creates new species","It guarantees survival","It removes competition everywhere"],"answer":0},{"question":"Why should endangered species be protected?","options":["Their populations may be at risk of disappearing","They are always dangerous","They cannot reproduce","They do not affect ecosystems"],"answer":0},{"question":"Which is a sensible school biodiversity activity?","options":["Record and classify plants seen around the school","Remove all insects","Collect every wild animal","Cut down shrubs"],"answer":0},{"question":"Why should repeated observations be compared?","options":["They can reveal patterns more reliably","They remove all differences","They make guesses correct","They prevent recording"],"answer":0},{"question":"A camel and a fish have different body features mainly because?","options":["They are adapted to different environments","They belong to the same habitat","They eat exactly the same food","They never interact with surroundings"],"answer":0}],"hard":[{"question":"Why is it risky to classify an organism using only one visible feature?","options":["Different organisms may share one feature but differ in many others","Every organism has only one feature","Classification never uses evidence","One feature always proves a group"],"answer":0},{"question":"A forest loses many flowering plants. Which effect is most likely first?","options":["Animals depending on those plants may lose food","All predators immediately disappear","Rocks change colour","The forest becomes aquatic"],"answer":0},{"question":"Why can a food web be more stable than a single food chain?","options":["Some consumers may have alternative food sources","A food web has no producers","Food webs contain no predators","Energy is not transferred"],"answer":0},{"question":"An animal has broad feet and padded soles. Which habitat is this most likely useful in?","options":["Sandy desert","Deep ocean","Frozen lake","Dense pond"],"answer":0},{"question":"Which conclusion is strongest after observing a bird with webbed feet?","options":["Its feet are suited for swimming","It can definitely fly long distances","It must eat only fish","It cannot live on land"],"answer":0},{"question":"Why are decomposers essential after organisms die?","options":["They return nutrients to the environment","They stop decomposition","They remove all oxygen","They become producers"],"answer":0},{"question":"A pond receives too much fertilizer runoff. What could happen?","options":["Algal growth may change and disrupt the ecosystem","Every fish becomes a producer","Water becomes a desert","Biodiversity always increases"],"answer":0},{"question":"Why can an invasive species threaten native biodiversity?","options":["It may compete for food, space or other resources","It always becomes a decomposer","It removes all non-living factors","It cannot reproduce"],"answer":0},{"question":"Which investigation best tests the effect of light on plant growth?","options":["Change light while keeping plant type, water and soil similar","Change light, water and soil together","Use a different species for each trial","Measure only leaf colour once"],"answer":0},{"question":"A student says, 'This animal lives in water, so it must be a fish.' Why is this reasoning weak?","options":["Other animals such as dolphins also live in water","All aquatic animals have scales","Fish are the only aquatic organisms","Water is not a habitat"],"answer":0},{"question":"Why is adaptation different from a temporary response?","options":["An adaptation is a useful inherited characteristic, while a temporary response can occur during life","They are exactly identical","Temporary responses are always genetic","Adaptations happen only in laboratories"],"answer":0},{"question":"If a species disappears from a food web, why might another population increase?","options":["Its predator may have lost a food source and the balance can shift","All populations always decrease together","Producers become extinct automatically","Decomposers stop working"],"answer":0},{"question":"Which evidence best supports that two organisms belong to different groups?","options":["Multiple consistent differences in important characteristics","One difference in colour","Their names","Where a student first saw them"],"answer":0},{"question":"Why should conservation consider habitats as well as individual animals?","options":["Protecting habitat preserves food, shelter and breeding conditions","Animals never use habitats","Habitats are non-living only","Individual protection always restores ecosystems"],"answer":0},{"question":"A scientist finds an organism in a very salty lake with unusual tolerance. What is the best scientific response?","options":["Record its features and investigate how those features support survival","Immediately invent a new species name without evidence","Ignore the observation","Assume it can live everywhere"],"answer":0},{"question":"Why can biodiversity improve ecosystem resilience?","options":["Different organisms can perform different roles and provide alternatives after disturbance","More species always means no competition","Diversity prevents every change","Only predators contribute to resilience"],"answer":0},{"question":"A forest has producers, herbivores, carnivores and decomposers. What does this show?","options":["Different trophic roles are interconnected","Only animals matter","Plants do not provide energy","Decomposers are consumers only"],"answer":0},{"question":"Why does removing a top predator sometimes increase herbivore numbers?","options":["Reduced predation can allow prey populations to rise","Predators produce food for herbivores","Plants stop growing immediately","Herbivores become decomposers"],"answer":0},{"question":"Which is the best reason to use scientific names or clear group names in classification?","options":["They reduce confusion when different organisms have local names","They make organisms identical","They remove the need for observation","They guarantee every species is known"],"answer":0},{"question":"A plant has waxy leaves and a thick stem in a dry region. What inference is most reasonable?","options":["These features may reduce water loss or store water","The plant must be aquatic","It cannot photosynthesize","It must be a carnivore"],"answer":0},{"question":"Why might microorganisms be found in apparently clean environments?","options":["Microscopic life is widespread and can live under many conditions","Microorganisms occur only in dirty places","Clean environments contain no life","All microorganisms need sunlight"],"answer":0},{"question":"Why can an ecosystem change after one species is removed even if many species remain?","options":["Species are linked through feeding and other interactions","Species are completely independent","Only non-living factors matter","Biodiversity means no interaction"],"answer":0},{"question":"Which conservation action addresses habitat loss most directly?","options":["Restoring and protecting natural habitats","Collecting more souvenirs","Increasing hunting","Replacing native species with any crop"],"answer":0},{"question":"A student compares two habitats and finds one has more species. What should be checked before concluding it has greater biodiversity?","options":["Sampling area, effort and methods should be comparable","Only the largest animal should be counted","The weather must be ignored","Only plants should be counted"],"answer":0},{"question":"Why is a food chain represented with arrows?","options":["They show the direction of food and energy transfer","They show where animals live","They show body size","They show classification rank"],"answer":0}],"toohard":[{"question":"A desert plant has small leaves and a thick waxy surface. Which combination best explains these features?","options":["Reduced water loss and protection from drying","Better swimming and greater oxygen uptake","Increased hunting and camouflage only","Faster seed dispersal by water"],"answer":0},{"question":"A pond loses most aquatic plants. Which change is most likely?","options":["Herbivorous aquatic animals may lose food and oxygen conditions may change","All carnivores become producers","Water becomes non-living","Decomposers immediately disappear"],"answer":0},{"question":"Why is a controlled comparison important when testing habitat effects?","options":["It helps separate the effect of the chosen factor from other conditions","It guarantees the hypothesis","It removes all natural variation","It makes measurement unnecessary"],"answer":0},{"question":"A classification decision is based on feathers, wings and beak. Which conclusion is most justified?","options":["The organism is likely a bird, based on several shared features","It must live only in forests","It cannot be aquatic","It must eat seeds"],"answer":0},{"question":"A food web contains grass → deer → tiger and grass → rabbit → tiger. If deer numbers fall, what may happen?","options":["The tiger may rely more on rabbits, increasing pressure on rabbit numbers","Grass immediately disappears","The tiger becomes a producer","Rabbit numbers must become zero"],"answer":0},{"question":"Why is the loss of a decomposer community especially serious?","options":["Nutrients in dead material may not be recycled efficiently","Producers no longer need sunlight","Predators cannot see prey","Every organism becomes aquatic"],"answer":0},{"question":"Two species have similar body shapes but live in very different habitats. What should a student avoid?","options":["Assuming they must have identical adaptations","Comparing their features","Recording their habitats","Investigating their food"],"answer":0},{"question":"Which result would most strongly support habitat specialization?","options":["A species consistently survives and reproduces better under a particular set of conditions","An individual moves once","A species is seen in one place once","A habitat has a common colour"],"answer":0},{"question":"A conservation plan protects an animal but destroys its breeding habitat. What is the weakness?","options":["The plan may not protect the conditions needed for reproduction","Animal protection never matters","Breeding habitats are irrelevant","Predators will disappear automatically"],"answer":0},{"question":"Why can biodiversity decline without any species becoming immediately extinct?","options":["Population sizes and habitat ranges can shrink before extinction occurs","Biodiversity only changes at extinction","Decline means every species disappears","Biodiversity is just species names"],"answer":0},{"question":"A lake receives pollutants that kill insects. Which food-web effect is plausible?","options":["Animals that depend on those insects may decline","All producers increase forever","Every fish becomes a decomposer","Predators become plants"],"answer":0},{"question":"A learner wants to compare biodiversity in two school plots. Which design is strongest?","options":["Use equal-sized plots and the same observation method and effort","Use one large plot and one tiny plot","Count only the most colourful species","Visit one plot many times and the other once"],"answer":0},{"question":"Why does sampling effort matter in biodiversity surveys?","options":["More observation effort can reveal more species and affect counts","Effort never changes observations","Only weather matters","Sampling is not part of science"],"answer":0},{"question":"Which claim is too broad after observing one cactus?","options":["All desert plants store water in exactly the same way","This cactus has features suited to dry conditions","Its thick stem may help water storage","Its reduced leaves may limit water loss"],"answer":0},{"question":"Why can similar adaptations evolve in unrelated organisms?","options":["Similar environmental pressures can favour similar useful features","All organisms share identical ancestors","Habitats force exact body shapes","Adaptations are not linked to survival"],"answer":0},{"question":"A new organism has traits shared with two groups. What should classification do first?","options":["Collect more evidence about multiple characteristics","Choose the group with the nicer name","Use only body colour","Ignore the unusual traits"],"answer":0},{"question":"Why should ecosystem conclusions be stated with conditions?","options":["Ecosystem responses depend on species and environmental context","Conditions never matter","It makes science weaker","Every ecosystem behaves identically"],"answer":0},{"question":"A forest has many plant species but heavy habitat fragmentation. What can be inferred cautiously?","options":["Current diversity may remain high while connectivity and long-term survival are at risk","Fragmentation always increases biodiversity","All species are safe","Fragmentation affects only rocks"],"answer":0},{"question":"Which evidence best supports a claim that a feature is an adaptation?","options":["The feature is associated with improved survival or reproduction under relevant conditions","The feature looks unusual","A single individual has it","The feature is colourful"],"answer":0},{"question":"Why is conservation of genetic diversity important within a species?","options":["It can help populations retain variation that may support future adaptation","All individuals must be identical","Genetic variation stops reproduction","Genetic diversity only matters in plants"],"answer":0},{"question":"What is the best interpretation of a simulation used to study ecosystems?","options":["It illustrates a model of relationships, not every detail of a real ecosystem","It exactly predicts every real ecosystem","It replaces field observations","It proves a hypothesis automatically"],"answer":0},{"question":"A food web has several paths from plants to a predator. Why might that provide some resilience?","options":["The predator may switch among available prey when one changes","The predator never needs food","Plants are consumers","Multiple paths remove all competition"],"answer":0},{"question":"Why is habitat restoration often more sustainable than only relocating individual animals?","options":["It addresses the environmental conditions required for continued populations","Animals never need habitats","Relocation always increases biodiversity","Restoration removes all predators"],"answer":0},{"question":"A species is declining because of pollution and habitat loss together. Why is a single-factor solution likely insufficient?","options":["More than one pressure is contributing to the decline","One pressure always causes every decline","Habitat and pollution are identical","Species decline cannot have multiple causes"],"answer":0},{"question":"Which statement best distinguishes biodiversity from ecosystem balance?","options":["Biodiversity describes variety of life, while ecosystem balance concerns interactions and stability","They are exactly the same term","Biodiversity means only balance","Ecosystem balance means number of species only"],"answer":0}],"extreme":[{"question":"A habitat has 20 plant species, but after a disturbance only 12 remain. Why should biodiversity loss not be judged only by species count?","options":["Population size, functional roles and genetic variation may also have changed","Species count is never useful","All remaining species are identical","Only predators matter"],"answer":0},{"question":"Two habitats contain the same number of species but different abundances. What could differ?","options":["Their ecological structure and resilience","Their number of species","The existence of habitats","The definition of biodiversity"],"answer":0},{"question":"A food web loses a producer used by three herbivores. Why can the effect spread to higher trophic levels?","options":["Reduced primary food can lower several herbivore populations and then affect predators","Only plants are affected","Predators never depend on herbivores","Energy increases when producers disappear"],"answer":0},{"question":"A student claims a feature is an adaptation because it is useful. What additional reasoning is needed?","options":["Evidence that the feature is a heritable characteristic associated with survival or reproduction","Proof that it is colourful","Proof that it appeared once","A larger drawing"],"answer":0},{"question":"Why can a species be threatened even when some individuals remain common in one location?","options":["Its total range or populations elsewhere may be shrinking and its habitat may be fragmented","One location represents the entire species","Common individuals cannot be threatened","Threat status depends only on body size"],"answer":0},{"question":"A conservation programme increases the number of animals but reduces genetic diversity. What long-term concern remains?","options":["Low genetic diversity may reduce adaptive potential and increase vulnerability","More animals always solve every problem","Genetic diversity has no role","Population size and genetics are identical"],"answer":0},{"question":"A forest edge has the same number of species as the forest interior but fewer pollinators. What does this illustrate?","options":["Species count alone may miss changes in ecological functions","Pollinators are not part of biodiversity","Edges always have greater biodiversity","Function and diversity are unrelated"],"answer":0},{"question":"Why can invasive species cause indirect effects even without eating native species?","options":["Competition or habitat modification can change resources available to native organisms","Only predation causes ecological effects","Invasive species cannot interact with habitats","Indirect effects are impossible"],"answer":0},{"question":"A pond restoration adds native plants but leaves severe pollution unchanged. Why may recovery remain limited?","options":["Pollution can continue to stress organisms and prevent stable populations","Plants remove every pollutant automatically","Pollution never affects aquatic life","Native plants are always sufficient"],"answer":0},{"question":"A classification key groups organisms by several traits. Why is this stronger than using one trait?","options":["Multiple traits provide more reliable evidence for distinguishing groups","More traits always make organisms identical","One trait is never useful","Classification cannot use traits"],"answer":0},{"question":"A student observes frogs only during rainy evenings. What is the strongest interpretation?","options":["Observation timing may reflect activity patterns, so surveys at other times may be needed","Frogs exist only at night","Rain creates frogs","One observation proves habitat preference"],"answer":0},{"question":"Why might two ecosystems with identical species lists still function differently?","options":["Species abundances, interactions and environmental conditions can differ","Species lists always determine function completely","Abiotic factors never matter","All ecosystems with the same species are identical"],"answer":0},{"question":"A drought removes most flowers in a habitat. Which chain of effects is plausible?","options":["Pollinators lose resources, fruit production may fall, and dependent animals may be affected","Predators become producers","Water loss increases biodiversity automatically","Decomposers stop needing food"],"answer":0},{"question":"Why should conservation priorities sometimes protect common species as well as rare ones?","options":["Common species can play important ecosystem roles and may decline rapidly under new pressures","Only rare species affect ecosystems","Common species cannot become threatened","Conservation concerns only species with small body size"],"answer":0},{"question":"A species survives in captivity but declines in the wild. What does this suggest about conservation?","options":["Captive survival alone does not prove that wild habitat conditions are suitable","Captivity always reproduces natural conditions","Wild habitats are irrelevant","The species is necessarily extinct"],"answer":0},{"question":"A model predicts a food-web collapse after removing one species, but field data show little change. What is the best next step?","options":["Examine model assumptions and compare them with real interactions and measurements","Change the field data","Assume the model is infallible","Ignore the discrepancy"],"answer":0},{"question":"Why is biodiversity often measured using both richness and abundance?","options":["Richness counts types while abundance shows how populations are distributed","Both measure exactly the same thing","Abundance replaces species identification","Richness ignores species"],"answer":0},{"question":"A habitat corridor connects two fragmented forests. What ecological benefit is most plausible?","options":["It can improve movement and gene flow between populations","It guarantees no predators","It removes every invasive species","It makes habitats identical"],"answer":0},{"question":"Why can loss of one keystone-like species have an outsized effect?","options":["Its ecological role may influence many interactions relative to its abundance","All species have identical roles","Keystone species are always the largest","Only producers can affect food webs"],"answer":0},{"question":"A protected area has many species but poor water quality. What conclusion is most careful?","options":["Protection can conserve habitat while water pollution still limits ecosystem health","Protected areas automatically eliminate pollution","Species count proves the ecosystem is healthy","Water quality is unrelated to biodiversity"],"answer":0},{"question":"A learner finds more species after extending survey time. What does this suggest about the survey?","options":["Sampling effort influences detected richness and should be standardized","The extra species appeared because of observation","Longer surveys are always inaccurate","Species can be created by counting"],"answer":0},{"question":"Why is restoration using native species usually preferred to introducing unrelated species?","options":["Native species are more likely to fit existing food webs and local conditions","Unrelated species always improve ecosystems","Native species cannot reproduce","All species have identical ecological roles"],"answer":0},{"question":"Two species compete for the same food. Why might one species persist if a second food source becomes available?","options":["Resource availability can reduce direct competition and alter coexistence","Competition always causes extinction","Food sources are irrelevant","The species become decomposers"],"answer":0},{"question":"A protected species increases in number but its prey declines sharply. Why should conservation management continue monitoring both?","options":["Increasing predator numbers can change food-web relationships and create new pressures","Only protected species matter","Prey populations never affect predators","Monitoring should stop after recovery"],"answer":0},{"question":"What is the strongest overall principle for studying biodiversity scientifically?","options":["Use clear classification, comparable observations, evidence from multiple sources and cautious conclusions","Rely on one observation and broad claims","Count only large organisms","Choose conclusions before collecting evidence"],"answer":0}],"6-science-mindful-eating-a-path-to-a-healthy-body":{"easy":[{"question":"Easy Q1: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q2: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q3: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q4: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q5: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q6: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q7: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q8: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q9: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q10: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q11: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q12: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q13: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q14: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q15: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q16: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q17: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q18: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q19: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q20: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q21: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q22: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q23: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q24: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Easy Q25: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0}],"medium":[{"question":"Medium Q1: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q2: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q3: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q4: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q5: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q6: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q7: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q8: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q9: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q10: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q11: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q12: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q13: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q14: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q15: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q16: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q17: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q18: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q19: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q20: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q21: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q22: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q23: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q24: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Medium Q25: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0}],"hard":[{"question":"Hard Q1: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q2: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q3: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q4: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q5: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q6: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q7: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q8: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q9: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q10: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q11: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q12: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q13: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q14: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q15: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q16: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q17: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q18: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q19: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q20: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q21: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q22: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q23: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q24: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Hard Q25: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0}],"toohard":[{"question":"Toohard Q1: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q2: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q3: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q4: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q5: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q6: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q7: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q8: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q9: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q10: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q11: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q12: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q13: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q14: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q15: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q16: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q17: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q18: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q19: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q20: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q21: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q22: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q23: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q24: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Toohard Q25: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0}],"extreme":[{"question":"Extreme Q1: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q2: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q3: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q4: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q5: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q6: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q7: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q8: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q9: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q10: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q11: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q12: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q13: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q14: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q15: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q16: minerals is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q17: water is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q18: roughage is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q19: hygiene is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q20: digestion is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q21: balanced diet is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q22: proteins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q23: carbohydrates is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q24: fats is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0},{"question":"Extreme Q25: vitamins is related to healthy eating?","options":["Yes","No","Only animals","Only plants"],"answer":0}]}};
