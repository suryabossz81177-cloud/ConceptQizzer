...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!document.getElementById("chapterReader")) {
    ...
    document.getElementById("readerPlay").onclick = () => {
      ...
      utterance.rate = 0.9;
      utterance.onstart = () => status.textContent = "🔊 Reading chapter...";
      utterance.onend = () => status.textContent = "✅ Reading finished.";
      window.speechSynthesis.speak(utterance);
    };

    document.getElementById("readerPause").onclick = () => {
      if ("speechSynthesis" in window) {
    ...
    document.getElementById("readerStop").onclick = () => {
      if ("speechSynthesis" in window) {
        window.speechSynthesis.cancel();
  ...
  if (!document.getElementById("doubtQuestion")) {
    ...
    const ask = document.getElementById("askDoubtBtn");
    const question = document.getElementById("doubtQuestion");
    const answer = document.getElementById("doubtAnswer");
    ...
    ask.onclick = () => {
      ...
      if (!q) {
        ...
        answer.innerHTML = "<strong>⚠️ Please type a question.</strong>";
        return;
      }

      const chapter = window.CQ_ACTIVE_CHAPTER;

      if (!chapter) {
        answer.style.display = "block";
        answer.innerHTML = "<strong>⏳ Chapter is still loading. Try again.</strong>";
        return;
      }

      const words = q.replace(/[?.,!]/g, " ")
        .split(/\s+/)
        .filter(w => w.length >= 4);
      ...
      (chapter.sections || []).forEach(section => {
        ...
        const score = words.reduce(
          (n, word) => n + (searchable.includes(word) ? 1 : 0), 0
        );
      ...
      if (!best) {
        answer.innerHTML = `
          <h3>🤔 I couldn't find a direct answer.</h3>
          <p>Ask about a concept, definition, example, or important point from this chapter.</p>`;
        return;
      }

      const blocks = (best.blocks || []).slice(0, 3);

      answer.innerHTML = `
        <h3>📚 ${best.title || "Chapter Section"}</h3>
        ${
          blocks.length
            ? blocks.map(b => `
                <div style="margin:10px 0;">
                  <strong>${b.title || b.type || "Explanation"}</strong>
                  <p>${b.text || ""}</p>
                </div>`).join("")
            : `<p>${best.content || "See this section in the notes above."}</p>`
        }`;
    };
  }
});
